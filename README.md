 ...
 * Run command `brew update` to ensure you have the latest copies of the formulae
 * Run command `brew install kvm` to install KVM. This also automatically install the latest KRE package from our release feed.
  * If you'd like to run on the bleeding edge, you can use the `brew install kvm --HEAD` command to get the absolute latest version.
