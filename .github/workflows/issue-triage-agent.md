...
- When lookup status is not `true`, the current type is unknown. Preserve a
  ...
  `unknown (type lookup unavailable)`. A type recommendation in the skill's
  internal decision record is classification evidence only in this state; it
  never authorizes a `set-issue-type` call or a claim that a type was applied.
...
summary. Do not create a second workflow-specific recommendation format, and do
not include the internal decision record in the public comment.
...
- When lookup status is not `true`, ignore the record's type action for
  mutation. In the public summary, report a non-empty type from `issue_read` as
  preserved; otherwise replace only the summary's type value with
  `unknown (type lookup unavailable)`. Never call or claim `set-issue-type`.
