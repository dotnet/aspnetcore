...
and its lookup status. If the initial lookup failed, also tell the skill whether
the fallback `issue_read` call returned a non-empty current type. The workflow
independently owns the mutation gate:

- When lookup status is `true`, preserve a non-empty type or recommend a type
  only when the current type is empty.
- When lookup status is not `true`, a non-empty type returned by `issue_read`
  is trusted fallback evidence and must be preserved. Otherwise the type
  remains unknown, so the skill must use `Unknown (lookup unavailable)` in its
  internal decision record and `unknown (type lookup unavailable)` in the
  summary. Neither state authorizes a `set-issue-type` call or a claim that a
  type was applied.
...
Before emitting any safe output, have the skill produce exactly one internal
`### Triage Decision` record in its required four-line format (`Area`, `Type
action`, `Subtype`, and `Duplicate`), followed by its complete semantic triage
summary. The type action may be `Unknown (lookup unavailable)` only when the
trusted lookup failed and no non-empty fallback type was retrieved. Do not
create a second workflow-specific recommendation format, and do not include the
internal decision record in the public comment.

Treat the decision record as classification output, then independently enforce
the workflow-owned type lookup gate before translating it into safe outputs:

- When lookup status is `true`, use the record's type action subject to the
  current-type preservation rules below.
- When lookup status is not `true`, independently refuse type mutation. Require
  the record and public summary to preserve a non-empty type from `issue_read`,
  or to use the skill's exact unknown-type forms when no such type was
  retrieved. Never call or claim `set-issue-type`.

Keep the skill's area, subtype, duplicate, regression, and summary decisions
otherwise intact. Separately apply the workflow-owned vulnerability gate above
to decide whether any comment may be posted. Then translate the single decision
into the existing safe outputs in the exact order below.

## Step 7: Apply Labels, Type, and Post the Comment

Order of operations matters. Do these in this exact order:

1. **Use the single skill decision record** to decide the labels and comment
...
6. **Use the skill's complete semantic triage summary as the comment.** Never
   include the internal `### Triage Decision` record. The applied labels and
   issue type are visible in the issue's label sidebar; do not list them inside
   the comment body.
