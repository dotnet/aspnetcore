...
function Assert-PresentationLayout
{
    ...
    Assert-True (-not ($Body -match "(?i)<(?:details|table|a|br)\b|\]\(|https?://|(?<![\w])@[A-Za-z0-9]")) "Presentation must not add HTML, links or mentions."
...
function Assert-PresentationTablesAndFields
{
    ...
    foreach ($viewName in $presentationViews.Keys)
    {
        ...
        for ($index = 0; $index -lt $items.Count; $index++)
        {
            ...
            $reference = "``dotnet/aspnetcore#$($item.number)``"
    ...
    $references = @([regex]::Matches($Body, '`dotnet/aspnetcore#[0-9]+`') | ForEach-Object Value)
    Assert-True (($references -join ",") -ceq ($expectedReferences -join ",")) "Each qualified code reference must occur once, in original view order."
}
...
function Assert-PresentationScopePlacement
{
    ...
    else
    {
        ...
        foreach ($item in $items)
        {
            $row = @($Body.Split("`n") | Where-Object { $_.Contains("``dotnet/aspnetcore#$($item.number)``") })
...
try
{
    ...
    foreach ($name in @("changed-cell", "duplicate-reference"))
    {
        Invoke-PresentationCase "PresentationCanonicalRoundTrip/$name" {
            $canonical = Invoke-PinnedOutputSanitizer -Content (Invoke-Renderer -Pulse $normal)
            $cell = "| First review candidate |"
            Assert-True ($canonical.Contains($cell)) "The candidate title cell must exist before exercising table-specific tampering."
            $replacement = if ($name -ceq "changed-cell")
            {
                "| Altered title |"
            }
            else
            {
                '| First review candidate `dotnet/aspnetcore#101` |'
            }
            $tampered = $canonical.Replace($cell, $replacement)
