...
Also check for recently closed (not merged) `[test-quarantine]` PRs from the past 30 days that targeted the same test — if a repository member closed the PR with a comment explaining why the quarantine/unquarantine should not happen, skip that test. See the "Important Rules" section for details.
