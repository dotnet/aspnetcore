...
- **`[QuarantinedTest]` attributes must reference a GitHub issue URL that *ultimately resolves* to a numeric issue number** (e.g., `https://github.com/dotnet/aspnetcore/issues/12345`). For a newly created issue (Case A) you write the `#{temporary_id}` token while editing (see below); the framework resolves it to the numeric URL before the PR is opened, so the final committed code is numeric. Never write placeholder strings, descriptive text, or any other non-numeric identifier — the only permitted non-numeric value is the required `#{temporary_id}` token.
  ...
  - **The `temporary_id` must be exactly `aw_` followed by 3 to 12 characters from `[A-Za-z0-9_]` (so the whole token is 6–15 characters, e.g. `aw_http2ign`, `aw_qgridsort`).** This limit is enforced by the framework. If you pass a `temporary_id` that is too long or otherwise malformed, the framework **silently discards it, auto-generates a *different* id, and registers your new issue under that auto-generated id** — so the `#{temporary_id}` token you wrote into the code no longer matches any registered id, is treated as a malformed reference, and is **committed verbatim as a broken placeholder** (this is exactly how `#aw_quickgrid_sort` leaked into a `[QuarantinedTest]` attribute). Keep the slug short and abbreviate: e.g. for `SortByTypeMismatchVirtualizedShowsClearError` use `aw_qgridsort` (9 chars), **not** `aw_quickgrid_sort` (14 chars — over the limit). The identical `temporary_id` must be used for the `create_issue` call, the `add_comment` `item_number`, the `#{temporary_id}` in the attribute, and the `Associated issue: #{temporary_id}` in the PR body — pick one short, valid slug up front and reuse it everywhere.
...
1. **Create a test-failure issue** via `create_issue` with a `temporary_id` (e.g., `aw_http2ign`). The `temporary_id` **must** be `aw_` followed by 3 to 12 characters from `[A-Za-z0-9_]` (whole token 6–15 chars); choose a **short** abbreviated slug — a too-long id (e.g. `aw_quickgrid_sort`, 14 chars) is silently replaced by an auto-generated id, which orphans the `#{temporary_id}` you write into code and leaves a broken placeholder. Use this exact structure:
