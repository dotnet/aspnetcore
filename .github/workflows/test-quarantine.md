    ...
    - name: Fetch re-quarantine PRs
      ...
      run: |
        ...
        for pr in prs:
            ...
            for f in files:
                patch = f.get("patch", "")
                if any(line.startswith("+") and "[QuarantinedTest]" in line for line in patch.split("\n")):
                    quarantined_files.append(f["filename"])
            requarantine_data.append({
                "number": pr["number"],
                "title": pr["title"],
                "files_adding_quarantine": quarantined_files
            })
...
- The test has **never been re-quarantined**. A test is considered re-quarantined if there exists any merged PR in the repository that either has "Re-quarantine" (case-insensitive) in the title, or has the `re-quarantine` label, and that PR added a `[QuarantinedTest]` attribute to the same test method, test class, or test assembly. To check this:
  ...
  Read the file `/tmp/requarantine-prs.json` (generated by the pre-activation step). This file contains all merged re-quarantine PRs with their changed files that add `[QuarantinedTest]` attributes. For each entry, check if any filename in `files_adding_quarantine` matches the test's source file path. If so, this test must be permanently excluded from automated unquarantining. Only a human may unquarantine such a test.
