...
- It has **real run evidence**: at least one **passing** result for that exact fully-qualified name within the past 30 days. A test with **zero** pass *and* zero fail rows for its fully-qualified name (it never actually ran, or produced only "other"/skipped outcomes) has no evidence of reliability — **fail closed and do NOT unquarantine it.** Do not borrow another test's pass count to satisfy this requirement.
