...
- **`[QuarantinedTest]` attributes in final committed code must reference a real GitHub issue URL** with a numeric issue number (e.g., `https://github.com/dotnet/aspnetcore/issues/12345`). Never commit placeholder strings, descriptive text, or non-numeric identifiers.
  - **For a newly created quarantine issue (Case A), you MUST write the `#aw_<temporary_id>` reference — never a literal numeric issue number.** The issue's real number is assigned by the framework *after* the agent finishes, so it is impossible for you to know it while editing code. Write `[QuarantinedTest("https://github.com/dotnet/aspnetcore/issues/#aw_myid")]` where `myid` is the exact `temporary_id` you passed to the corresponding `create_issue` call. The framework resolves `#aw_myid` to the real numeric URL before opening the PR, so the final committed code will contain the numeric URL.
  - **A literal numeric issue URL is allowed ONLY when reusing an already-existing tracking issue (Case B re-quarantine), and only after you have confirmed in this run that the issue exists, is open, and carries the `test-failure` label.** Never write a literal number for an issue you created (or will create) in this run.
  - **Never** use placeholder text like `TODO`, `TBD`, or descriptive strings.
- **Never guess, predict, probe for, or reverse-engineer a GitHub issue number.** Do not try to discover "what number my new issue will get" by listing issues, incrementing the latest issue/PR number, calling the GitHub or MCP issue/PR lookup APIs, or any similar technique. New-issue numbers are assigned asynchronously by the framework and are unknowable while you are editing code — the only correct way to reference a newly created issue is the `#aw_<temporary_id>` token (see above).
...
1. If `<ref>` is for an issue you created in this run, it **must** be `#aw_<temporary_id>` using the *exact* `temporary_id` you passed to a `create_issue` call in this same run. A bare number here is a bug — fix it before submitting.
2. If `<ref>` is a literal number, it **must** be an existing issue that you have confirmed in this run is open and labeled `test-failure` (Case B reuse only). If you cannot confirm that, do not submit the PR.
3. `<ref>` must never be a guessed or probed number, a `TODO`/`TBD`/placeholder, or any value derived from an issue/PR API or MCP lookup result.
