...
description: >-
  ...
  a good idea, whether to approve or decline an API request, changing/extending an existing API or a
  shipped default/convention, or checking naming/namespace/nullability/async/sealing/return-type/overload
...
4. **Apply the rules** from [references/conventions.md](references/conventions.md) to every member — naming, namespace, async+`CancellationToken`, nullability, return/parameter types, overloads vs optional params, sealing/extensibility, defaults, consistency, breaking-change/obsoletion. Inline highlights below.
...
- **Method families have a fixed contract** (matches the shipped surface): `Add*` registers services → returns `IServiceCollection`/feature builder, in `Microsoft.Extensions.DependencyInjection`; `Use*` adds middleware → `IApplicationBuilder`, in `Microsoft.AspNetCore.Builder`; `Map*` adds endpoints → endpoint convention builder, in `Microsoft.AspNetCore.Builder`; `With*` attaches endpoint metadata → returns the same builder. **These placements hold regardless of the feature's own namespace** so they light up with the common `using`. A proposed `Add*`/`Use*`/`Map*` that returns the wrong type, sits in the wrong namespace, or does the wrong job is a finding. See [conventions.md](references/conventions.md#patterns-confirmed-across-the-shipped-surface).

## Reviewing checklist

Need assessed (commonality + audience) · diff is against the real current surface · every member checked for naming/async/nullability/return-type/overload/sealing/namespace/compat · **every recommended change has a grounded Why and none defeats a stated goal of the proposal** · verdict matches the body · output follows [output-format.md](references/output-format.md).
