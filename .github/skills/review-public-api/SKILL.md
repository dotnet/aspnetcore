...
- **Async.** New async public methods return `Task`/`ValueTask` **with a `CancellationToken = default`** and flow it through; prefer `Task` over `ValueTask` for framework callback delegates unless a hot path justifies it; suffix async methods `Async`.
...
- **Consistency & placement.** Mirror sibling APIs (`Add*`/`With*`, `IServiceCollection` patterns, `IResult`/`IEndpointMetadataProvider`); put niche types in a feature sub-namespace, infrastructure in `*.Infrastructure`, concrete impls beside their interface.
