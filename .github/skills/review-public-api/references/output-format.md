...
- **Scenarios & environment:** the runtime environments this API runs in and whether it actually works there (e.g. static SSR vs interactive, Blazor Server vs WebAssembly, scaled-out, AOT/trimmed), and the **common vs rare** usage patterns (name the dominant one). Call out any environment where it silently doesn't work.
