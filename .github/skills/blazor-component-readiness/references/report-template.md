...
foundation. `--source-report` validates the source report and requires exact tracker parity for
requirement IDs, wording, scope, status, and evidence references. If the tracker declares the
source-report digest, those same live bytes must match the declaration. Omit the example
`--provenance-input` when the tracker declares no additional artifact digest; otherwise repeat it
once per live digest-bearing input.
...
`Review result` is a cautious display label for a review audience. It is derived, not judged: it is
