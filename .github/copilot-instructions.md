...
* For behavioral review findings and bug-fix verification, use the smallest faithful test path. Include the component or browser mechanism that owns or produces each disputed precondition, and observe the claimed user-visible result. Directly injected callbacks or events and isolated component tests prove only the downstream response, not that the real producer can reach that path.
