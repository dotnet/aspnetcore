...
if ($OutputFile) {
    # Discover all top-level src/ areas and compute unaffected ones
    $srcRoot = Join-Path $repoRoot "src"
    $allAreas = Get-ChildItem -Directory $srcRoot | Select-Object -ExpandProperty Name | Sort-Object
    $unaffectedAreas = $allAreas | Where-Object { -not $affectedAreas.Contains($_) }

    # Generate .props file with Remove lines for unaffected areas
    $lines = [System.Collections.Generic.List[string]]::new()
    $lines.Add('<Project>')
    $lines.Add('  <!-- Auto-generated by GetAffectedTestAreas.ps1. Do not edit. -->')
    $lines.Add('  <PropertyGroup>')
    $lines.Add("    <AffectedTestAreas>$result</AffectedTestAreas>")
    $lines.Add('  </PropertyGroup>')
    $lines.Add('  <ItemGroup>')
    foreach ($area in $unaffectedAreas) {
        $lines.Add("    <ProjectsWithTestsSubset1 Remove=`"`$(RepoRoot)src\$area\**\*`" />")
        $lines.Add("    <ProjectsWithTestsSubset2 Remove=`"`$(RepoRoot)src\$area\**\*`" />")
    }
    $lines.Add('  </ItemGroup>')
    $lines.Add('</Project>')
