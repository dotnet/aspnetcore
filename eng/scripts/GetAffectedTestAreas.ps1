...
if ($OutputFile) {
    $propsContent = @"
<Project>
  <!-- Auto-generated by GetAffectedTestAreas.ps1. Do not edit. -->
  <PropertyGroup>
    <AffectedTestAreas>$result</AffectedTestAreas>
  </PropertyGroup>
</Project>
"@
    $outputDir = Split-Path $OutputFile -Parent
    if (-not (Test-Path $outputDir)) { New-Item -ItemType Directory -Path $outputDir -Force | Out-Null }
    $propsContent | Set-Content -Path $OutputFile -NoNewline -Encoding UTF8
}
