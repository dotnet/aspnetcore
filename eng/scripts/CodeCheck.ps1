...
try {
    ...
    $changedFiles = & cmd /c 'git --no-pager diff --ignore-space-at-eol --ignore-space-change --name-only 2>nul'
    ...
    if ($changedFiles) {
        foreach ($file in $changedFiles) {
            ...
            & git --no-pager diff --ignore-space-at-eol --ignore-space-change $filePath
        }
    }
}
finally {
