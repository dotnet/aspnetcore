...
internal static partial class EvidenceIdentity
{
        ...
        long maximumNupkgBytes = MaximumNupkgBytes)
    {
        ...
        var nupkgBytes = ReadNupkgSnapshot(
            nupkgStream,
            maximumNupkgBytes);
        return ReadPackageIdentityFromSnapshot(nupkgBytes);
    }

    internal static PackageIdentity ReadPackageIdentity(byte[] nupkgBytes)
    {
        ArgumentNullException.ThrowIfNull(nupkgBytes);
        if (nupkgBytes.LongLength > MaximumNupkgBytes)
        {
            throw new InvalidDataException(
                $"EVID006: nupkg input exceeds {MaximumNupkgBytes} bytes.");
        }

        return ReadPackageIdentityFromSnapshot(nupkgBytes.ToArray());
    }

    private static byte[] ReadNupkgSnapshot(
        Stream nupkgStream,
        long maximumNupkgBytes)
    {
        var length = nupkgStream.Length;
        if (length > maximumNupkgBytes || length > int.MaxValue)
        {
            throw new InvalidDataException(
                $"EVID006: nupkg input exceeds {maximumNupkgBytes} bytes.");
        }
        ...
        {
            var read = nupkgStream.Read(bytes, offset, bytes.Length - offset);
            if (read == 0)
            {
                throw new InvalidDataException(
                    "EVID006: nupkg input changed while it was read.");
            }

            offset += read;
        }

        if (nupkgStream.ReadByte() != -1)
        {
            throw new InvalidDataException(
                $"EVID006: nupkg input grew while it was read or exceeds " +
                $"{maximumNupkgBytes} bytes.");
        }

        return bytes;
    }

    private static PackageIdentity ReadPackageIdentityFromSnapshot(
        byte[] nupkgBytes)
    {
        var digest = new Sha256Digest(
            "sha256",
            Convert.ToHexStringLower(
                System.Security.Cryptography.SHA256.HashData(nupkgBytes)));
