...
internal static class ScorecardCommand
{
        ...
        Action? beforeReceiptPublish = null)
    {
        ...
        for (var index = 0; index < args.Length; index++)
        {
            switch (args[index])
            {
                case "--skill-dir":
                    ...
                    break;
                case "--checklist":
                    if (!TryReadValue(args, ref index, out checklistPath))
                    {
                        return MissingValue("--checklist", error);
                    }
        ...
        if (checklistPath is not null && string.IsNullOrWhiteSpace(checklistPath))
        {
            error.WriteLine("ERROR: --checklist requires a non-empty value.");
            return 1;
        }
        ...
        try
        {
            ...
            if (receiptPath is not null &&
                !string.Equals(
                    checklistPath,
                    Path.GetFullPath(layout.ChecklistPath),
                    StringComparison.Ordinal))
            {
                error.WriteLine("--receipt cannot be combined with a custom --checklist.");
                return 1;
            }
