...
internal static partial class EmbeddedDigestValidator
{
        ...
        ReportSnapshot? sourceReport)
    {
        ...
        AddRepositoryCommit(allowed, bundle.Assessment.Repository.Commit);
        foreach (var source in bundle.SourceLedgers)
        {
            allowed.Add(source.SourceLedgerSha256);
            AddRepositoryCommit(
                allowed,
                source.Ledger.RepositorySubject?.Repository.Commit);
            AddRepositoryCommit(
                allowed,
                source.Ledger.ComponentSubject?.Repository.Commit);
    ...
    private static void AddRepositoryCommit(
        ISet<string> allowed,
        string? commit)
    {
        if (commit?.Length == 64)
