...
- Preserve Blazor's inbound route precedence: compare segments from left to right; for simple segments, literal segments precede constrained parameters, unconstrained parameters, constrained catch-all parameters, and unconstrained catch-all parameters. Optionality is not a precedence tie-breaker: `/users/{id}` and `/users/{id?}` must be rejected as ambiguous rather than preferring the required parameter. Supplemental implementation/test references: [`RoutePrecedence.ComputeInbound`](../src/Http/Routing/src/Template/RoutePrecedence.cs) and [`RouteTableFactoryTests.ThrowsForOptionalParametersAndNonOptionalParameters`](../src/Components/Components/test/Routing/RouteTableFactoryTests.cs).
