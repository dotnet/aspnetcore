...
namespace RepoTasks
{
    public class VerifySignedRuntimeStoreBinaries : Task
    {
        ...
        {
            var foundUnsignedBinaries = false;
            foreach (var binary in Binaries)
            {
                var binaryName = binary.ItemSpec;
                if (!WinTrust.IsAuthenticodeSigned(binaryName))
                {
                    Log.LogError($"{binaryName} is not authenticode signed");
                    foundUnsignedBinaries = true;
                }
                else
                {
