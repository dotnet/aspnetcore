...
```csharp
[Display(ResourceType = typeof(MyResources), Name = "CustomerName")]
[Required(ErrorMessageResourceType = typeof(MyResources), ErrorMessageResourceName = "RequiredError")]
...
Furthermore, the validation metadata source generator currently ignores the `DisplayAttribute.ResourceType` property, meaning that display names currently cannot be localized at all (see https://github.com/dotnet/aspnetcore/issues/65647).
...
- **Optional: Convention-based key selection** — support convention-based resource key selection for attributes that do not have an explicit `ErrorMessage` set.
- 
### 2.2. Non-functional
...
- Support async localization. Some localizer implementations might benefit from it. However, there are multiple reasons against:
  ...
  - Even if this changes (https://github.com/dotnet/aspnetcore/issues/7680), unless the sync code path is removed, both the sync and async executions should provide same error messages.
