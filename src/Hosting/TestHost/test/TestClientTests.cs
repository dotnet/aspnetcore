...
namespace Microsoft.AspNetCore.TestHost
{
    public class TestClientTests
    {
        ...
        public async Task PutAsyncWorks()
        {
            ...
            var response = await client.PutAsync("http://localhost:12345", content).WaitAsync(DefaultTimeout);

            // Assert
            Assert.Equal("Hello world PUT Response", await response.Content.ReadAsStringAsync().WaitAsync(DefaultTimeout));
        }
        ...
        public async Task PostAsyncWorks()
        {
            ...
            // Act
            var content = new StringContent("Hello world");
            var response = await client.PostAsync("http://localhost:12345", content).WaitAsync(DefaultTimeout);

            // Assert
            Assert.Equal("Hello world POST Response", await response.Content.ReadAsStringAsync().WaitAsync(DefaultTimeout));
        }
        ...
        public async Task ClientStreamingWorks()
        {
            ...
            // Act
            var response = await client.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead).WaitAsync(DefaultTimeout);

            await responseStartedSyncPoint.WaitForSyncPoint().WaitAsync(DefaultTimeout);
            responseStartedSyncPoint.Continue();

            var responseContent = await response.Content.ReadAsStreamAsync().WaitAsync(DefaultTimeout);
            ...
            var length = await responseContent.ReadAsync(buffer).AsTask().WaitAsync(DefaultTimeout);
            Assert.Equal("STARTED", Encoding.UTF8.GetString(buffer, 0, length));

            // Send content and finish request body
            await requestStream.WriteAsync(Encoding.UTF8.GetBytes("Hello world")).AsTask().WaitAsync(DefaultTimeout);
            await requestStream.FlushAsync().WaitAsync(DefaultTimeout);
            requestStreamSyncPoint.Continue();

            // Ensure content is received while request is in progress
            length = await responseContent.ReadAsync(buffer).AsTask().WaitAsync(DefaultTimeout);
            Assert.Equal("Hello world POST Response", Encoding.UTF8.GetString(buffer, 0, length));

            // Request is ending
            await requestEndingSyncPoint.WaitForSyncPoint().WaitAsync(DefaultTimeout);
        ...
        public async Task ClientStreaming_Cancellation()
        {
            ...
            // Write to request
            await requestStream.WriteAsync(Encoding.UTF8.GetBytes("SENT")).AsTask().WaitAsync(DefaultTimeout);
            await requestStream.FlushAsync().WaitAsync(DefaultTimeout);
            await responseReadSyncPoint.WaitForSyncPoint().WaitAsync(DefaultTimeout);

            // Cancel request. Disposing response must be used because SendAsync has finished.
            response.Dispose();
            responseReadSyncPoint.Continue();

            await responseEndingSyncPoint.WaitForSyncPoint().WaitAsync(DefaultTimeout);
        ...
        public async Task ClientStreaming_ResponseCompletesWithoutReadingRequest()
        {
            ...
            var writeTask = Task.Run(async () =>
            {
                try
                {
                    await requestStream.WriteAsync(Encoding.UTF8.GetBytes(new string('!', 1024 * 1024 * 50))).AsTask().WaitAsync(DefaultTimeout);
        ...
        public async Task ClientStreaming_ServerAbort()
        {
            ...
            await ExceptionAssert.ThrowsAsync<OperationCanceledException>(
                ...
                "Flush was canceled on underlying PipeWriter.").WaitAsync(DefaultTimeout);
