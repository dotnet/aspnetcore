...
namespace Microsoft.AspNetCore.ResponseCaching.Tests
{
    public class ResponseCachingMiddlewareTests
    {
        ...
        [Fact]
        public async Task FinalizeCacheHeaders_ReturnsFalse_IfBaseKeyContainsDelimiters()
        {
            var sink = new TestSink();
        ...
        [Fact]
        public async Task FinalizeCacheHeaders_ReturnsFalse_IfStorageVaryKeyContainsDelimiters()
        {
            var sink = new TestSink();
            var middleware = TestUtils.CreateTestMiddleware(testSink: sink, policyProvider: new ResponseCachingPolicyProvider());
            var context = TestUtils.CreateTestContext();
            ...
            // ShimResponseStream also calls AddResponseCachingFeature
            middleware.ShimResponseStream(context);
            // Set VaryByQueryKeys on the feature that ShimResponseStream added
            context.HttpContext.Features.Get<IResponseCachingFeature>().VaryByQueryKeys = new string[] { "*" };
            // Pre-set BaseKey so CreateBaseKey is skipped
