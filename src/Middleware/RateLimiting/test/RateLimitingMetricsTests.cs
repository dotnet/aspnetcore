...
public class RateLimitingMetricsTests
{
    ...
    public async Task Metrics_Rejected()
    {
        ...
        using var leaseRequestDurationRecorder = new Extensions.Metrics.InstrumentRecorder<double>(meterRegistry, RateLimitingMetrics.MeterName, "leased-request-duration");
        using var currentLeaseRequestsRecorder = new Extensions.Metrics.InstrumentRecorder<long>(meterRegistry, RateLimitingMetrics.MeterName, "current-leased-requests");
        using var currentRequestsQueuedRecorder = new Extensions.Metrics.InstrumentRecorder<long>(meterRegistry, RateLimitingMetrics.MeterName, "current-queued-requests");
        using var queuedRequestDurationRecorder = new Extensions.Metrics.InstrumentRecorder<double>(meterRegistry, RateLimitingMetrics.MeterName, "queued-request-duration");
        using var leaseFailedRequestsRecorder = new Extensions.Metrics.InstrumentRecorder<long>(meterRegistry, RateLimitingMetrics.MeterName, "lease-failed-requests");
