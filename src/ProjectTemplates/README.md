...
1. Run `src\ProjectTemplates\build.cmd -test -NoRestore -NoBuild -NoBuildDeps -configuration Release` (or the equivalent `src/ProjectTemplates/build.sh` command) to run all template tests.
...
On Windows, validation that does not require installers may also need `/p:GenerateInstallers=false`.
`-NoBuildInstallers` controls installer project selection by setting `BuildInstallers=false`, but it does not
necessarily suppress installer generation requested while producing runtime packs.
