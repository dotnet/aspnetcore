...
{
    public class SignalRTemplateTest
    {
        public SignalRTemplateTest(ProjectFactoryFixture projectFactory, ITestOutputHelper output)
        {
        ...
        [Fact]
        public async Task SignalRTemplate()
        {
            Project = await ProjectFactory.GetOrCreateProject("signalr", Output);

            var createResult = await Project.RunDotNetNewAsync("signalr");
