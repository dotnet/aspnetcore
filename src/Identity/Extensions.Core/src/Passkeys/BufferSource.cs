...
internal sealed class BufferSource : IEquatable<BufferSource>
{
    ...
    private string? _base64UrlString;
    ...
    {
        return new(bytes, base64UrlString: null);
    }
    ...
    public static BufferSource FromString(string value)
    {
        ...
        return new(buffer, base64UrlString: null);
    }

    /// <summary>
    /// Creates a new instance of <see cref="BufferSource"/> from a base64url-encoded string.
    /// </summary>
    public static BufferSource FromBase64UrlString(string base64UrlString)
    {
        var buffer = WebEncoders.Base64UrlDecode(base64UrlString);
        return new(buffer, base64UrlString);
    }

    private BufferSource(ReadOnlyMemory<byte> buffer, string? base64UrlString)
    {
        _bytes = buffer;
        _base64UrlString = base64UrlString;
    }
    ...
    /// </summary>
    /// <remarks>
    /// If originally constructed with a base64url-encoded string, this method will directly return that string.
    /// Otherwise, it will compute the base64url-encoded string from the byte buffer.
    /// </remarks>
    public string AsBase64UrlString()
    {
        return _base64UrlString ??= GetBase64UrlString(_bytes);

        static string GetBase64UrlString(ReadOnlyMemory<byte> bytes)
        {
