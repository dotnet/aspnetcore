...
public class InternalServerErrorOfTResultTests
{
    ...
    {
        // Arrange
        InternalServerError<ProblemDetails> MyApi() { throw new NotImplementedException(); }
        var builder = new RouteEndpointBuilder(requestDelegate: null, RoutePatternFactory.Parse("/"), order: 0);

        // Act
        PopulateMetadata<InternalServerError<ProblemDetails>>(((Delegate)MyApi).GetMethodInfo(), builder);
