...
{
    [Fact]
    public async Task WrappedNullValues_RespondTheSame_ForNullableReferenceTypes()
    {
    ...
    [Fact]
    public async Task WrappedNullValues_RespondTheSame_ForNullableValueTypes()
    {
        using var host = new HostBuilder()
        .ConfigureWebHost(webHostBuilder =>
        {
            webHostBuilder.Configure(app =>
            {
                ...
                app.UseEndpoints(e =>
                {
                    e.MapGet("/nullable", DataStruct? () => DataStruct.NullInstance);
                    e.MapGet("/results", IResult () => Results.Ok(DataStruct.NullInstance));
                    e.MapGet("/typed_results", Ok<DataStruct?> () => TypedResults.Ok<DataStruct?>(DataStruct.NullInstance));
                });
            }).UseTestServer();
        })
        .ConfigureServices(services =>
    ...
    {
        public static DataStruct? NullInstance => null;
    }
}
