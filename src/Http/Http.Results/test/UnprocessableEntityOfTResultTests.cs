...
public class UnprocessableEntityOfTResultTests
{
    ...
    {
        // Arrange
        UnprocessableEntity<ProblemDetails> MyApi() { throw new NotImplementedException(); }
        var builder = new RouteEndpointBuilder(requestDelegate: null, RoutePatternFactory.Parse("/"), order: 0);

        // Act
        PopulateMetadata<UnprocessableEntity<ProblemDetails>>(((Delegate)MyApi).GetMethodInfo(), builder);
