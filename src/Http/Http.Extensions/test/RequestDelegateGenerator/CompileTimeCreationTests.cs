...
""";
    ...
    // This test verifies that when an endpoint returns a type from another source generator,
    // the RDG emits a diagnostic and falls back to runtime generation instead of generating invalid code.
    [Fact]
    public async Task FallsBackToRuntimeFactoryWhenReturnTypeFromOtherSourceGenerator()
    {
        // Simulate the scenario where another source generator produces a type.
        // We'll create a mock incremental generator that generates a class with a method returning IResult.
        var source = """
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Routing;
...
public static class TestExtensions
{
    ...
    {
        // Reference a type generated by another source generator
...
""";
        ...
        // Create a mock source generator that generates a type
        var mockGenerator = new MockTypeIncrementalGenerator().AsSourceGenerator();
        ...
        // The RDG should emit a diagnostic for this endpoint because the return type is from another generator
        // and appears as an error type to the RDG
        var rdgResult = generatorRunResult.Results.First(r => r.Generator.GetGeneratorType().Name == "RequestDelegateGenerator");
        
        // Verify that a diagnostic was emitted (the endpoint should be filtered out)
        Assert.NotEmpty(rdgResult.Diagnostics);
        Assert.Contains(rdgResult.Diagnostics, d => d.Id == DiagnosticDescriptors.UnableToResolveMethod.Id);
        
        // Verify that no generated source contains the broken code patterns from the issue
        // (empty typeof(), invalid JsonTypeInfo<?>)
        foreach (var generatedSource in rdgResult.GeneratedSources)
        {
        ...
        // The compilation will have errors because GeneratedTypeFromOtherGenerator is not visible to the original compilation
        // This is expected - source generators cannot see each other's output
        // But the mock generator's output should be valid (it adds the type definition)
        var mockGeneratorResult = generatorRunResult.Results.First(r => r.Generator.GetGeneratorType().Name == "MockTypeIncrementalGenerator");
        Assert.Single(mockGeneratorResult.GeneratedSources);
        
        // The key assertion: The RDG should have emitted a diagnostic and NOT generated code that would cause 
        // compilation errors in its own generated source (like typeof() or JsonTypeInfo<?>)
    }

    // Mock incremental generator that generates a type to simulate another generator's output
    private class MockTypeIncrementalGenerator : IIncrementalGenerator
    {
