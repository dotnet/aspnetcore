...
{
    public static bool TryParseValues(StringValues values, IDictionary<string, string> store)
    {
        ...
        for (var i = 0; i < values.Count; i++)
        {
            ...
            {
                if (TryParseValue(value, ref index, false, out var parsedName, out var parsedValue))
                {
