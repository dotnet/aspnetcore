...
public class Startup
{
    ...
    public void ConfigureServices(IServiceCollection services)
    {
        ...
        // The app makes the source cookie its default scheme; AddDbsc detects that and
        // redirects the default authenticate scheme to the DBSC scheme, which authenticates the session
        // cookie before falling back to the source cookie.
        services.AddAuthentication(DbscNames.Source)
            ...
            // The DBSC handler + refresh/session cookie schemes.
            // The session TTL is read live from the debug state so it can be changed at runtime.
            .AddDbsc(DbscNames.Dbsc, o =>
            {
                o.SourceScheme = DbscNames.Source;
                ...
                o.ScopeSpecifications.Add(new DbscScopeRule
                {
                    Type = "exclude",
                    Domain = "*",
                    Path = "/debug",
            ...
            endpoints.MapPost("/login", async context =>
            {
                ...
                if (int.TryParse(form["ttl"], out var ttlSeconds) && ttlSeconds is > 0 and <= 86400)
                {
                    ...
                    context.RequestServices
                        .GetRequiredService<IOptionsMonitorCache<DbscOptions>>()
                        .Clear();
                }
