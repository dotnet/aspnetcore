...
/// </summary>
internal sealed class PostConfigureDbscDerivedCookieOptions : IPostConfigureOptions<CookieAuthenticationOptions>
{
    ...
    public PostConfigureDbscDerivedCookieOptions(
    ...
    public void PostConfigure(string? name, CookieAuthenticationOptions options)
    {
        ...
        if (schemes.RefreshSchemes.TryGetValue(name, out var refreshDbscScheme))
        {
            var dbscOptions = _dbscOptions.Get(refreshDbscScheme);
            CopyFromSource(dbscOptions.SourceScheme, options);
            ...
            options.Cookie = CreatePathBaseScopedCookie(options.Cookie, ResolveRefreshCookiePath(dbscOptions));
            ...
            ForwardSourceValidatePrincipal(options, dbscOptions.SourceScheme);
        }
        else if (schemes.SessionSchemes.TryGetValue(name, out var sessionDbscScheme))
        {
            var dbscOptions = _dbscOptions.Get(sessionDbscScheme);
            CopyFromSource(dbscOptions.SourceScheme, options);
    ...
    private static string ResolveRefreshCookiePath(DbscOptions dbscOptions)
    {
        const string fallback = "/.well-known/dbsc";

        var refreshPath = dbscOptions.RefreshPath.Value;
