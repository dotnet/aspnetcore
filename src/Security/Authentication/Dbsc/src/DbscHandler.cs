...
/// Authentication uses this scheme; sign-out uses <see cref="DbscOptions.SourceScheme"/> and clears the derived DBSC cookies.
/// </summary>
[Experimental("ASP0031", UrlFormat = "https://aka.ms/aspnet/analyzer/{0}")]
public class DbscHandler : AuthenticationHandler<DbscOptions>, IAuthenticationRequestHandler
{
    private readonly DbscChallengeProtector _challengeProtector;
    private readonly DbscJwtValidator _jwtValidator;
    private readonly IOptionsMonitor<CookieAuthenticationOptions> _cookieOptionsMonitor;

    /// <summary>
    /// Initializes a new instance of <see cref="DbscHandler"/>.
    /// </summary>
    /// <param name="options">The monitor for <see cref="DbscOptions"/> instances.</param>
    ...
    public DbscHandler(
        IOptionsMonitor<DbscOptions> options,
    ...
    {
        _challengeProtector = new DbscChallengeProtector(dataProtectionProvider, logger.CreateLogger<DbscChallengeProtector>());
        _jwtValidator = new DbscJwtValidator(logger.CreateLogger<DbscJwtValidator>());
    ...
    /// </summary>
    protected new DbscEvents Events
    {
    ...
    protected override Task<object> CreateEventsAsync() => Task.FromResult<object>(new DbscEvents());

    /// <summary>
    /// Authenticates normal requests using the session cookie scheme first, then the registration source scheme.
    /// </summary>
    /// <returns>A successful result restamped with the DBSC scheme, or <see cref="AuthenticateResult.NoResult"/>.</returns>
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        var result = await AuthenticateSchemeAsync(Options.SessionScheme);
        if (result is not null)
        {
            return result;
        }

        result = await AuthenticateSchemeAsync(Options.SourceScheme);
        if (result is not null)
        {
            return result;
        }
    ...
    private async Task<AuthenticateResult?> AuthenticateSchemeAsync(string? authenticationScheme)
    {
        if (string.IsNullOrEmpty(authenticationScheme) ||
            string.Equals(authenticationScheme, Scheme.Name, StringComparison.Ordinal))
        {
            return null;
        }
        ...
        var ticket = new AuthenticationTicket(result.Ticket.Principal, result.Ticket.Properties, Scheme.Name);
    ...
    private async Task HandleRegistrationAsync()
    {
        ...
        var proofHeader = Request.Headers[DbscConstants.Headers.Proof].ToString();
        ...
        var authResult = await Context.AuthenticateAsync(Options.SourceScheme);
        ...
        Context.Items[DbscCookieEvents.RegistrationExchangeItemKey] = true;
        await Context.SignOutAsync(Options.SourceScheme);

        await Events.SessionRegistered(new DbscRegisteredContext(Context, Scheme, Options, principal, properties));
        ...
        await JsonSerializer.SerializeAsync(Response.Body, instructions, DbscJsonContext.Default.SessionInstruction, Context.RequestAborted);
    }

    private async Task HandleRefreshAsync()
    {
        // Read session ID from header
        var sessionIdHeader = Request.Headers[DbscConstants.Headers.SessionId].ToString();
        ...
        if (string.IsNullOrEmpty(proofHeader))
        {
            ...
            Response.StatusCode = StatusCodes.Status403Forbidden;
            Response.Headers[DbscConstants.Headers.Challenge] = $"\"{challenge}\";id=\"{sessionIdHeader}\"";
            return;
        }
        ...
        if (!challengeValid)
        {
            ...
            Response.StatusCode = StatusCodes.Status403Forbidden;
            Response.Headers[DbscConstants.Headers.Challenge] = $"\"{retryChallenge}\";id=\"{sessionIdHeader}\"";
            return;
        }
        ...
        await Events.SessionRefreshed(new DbscRefreshedContext(Context, Scheme, Options, authResult.Principal, properties));
    ...
    private CookieAuthenticationOptions ResolveSessionCookieOptions()
    {
        ...
        {
            throw new InvalidOperationException(
                $"{nameof(DbscOptions.SessionScheme)} must be configured to build a DBSC session instruction.");
        }
