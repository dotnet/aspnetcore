...
/// <summary>
/// A <see cref="CookieAuthenticationEvents"/> decorator that implements the explicit integration contract
/// with a DBSC source cookie scheme when its cookie options set <c>EventsType</c>. It triggers DBSC
/// registration on sign-in and forwards every event to the application's DI-resolved inner events.
/// </summary>
internal sealed class DbscCookieEvents : CookieAuthenticationEvents
{
    ...
    public DbscCookieEvents(string dbscScheme, CookieAuthenticationEvents innerEvents, Type? innerEventsType)
    {
    ...
    public override async Task SigningIn(CookieSigningInContext context)
    {
        ...
        await DbscRegistrationHeader.Emit(context.HttpContext, context.Principal, context.Properties, _dbscScheme);
    }
    ...
    /// </summary>
    internal const string RegistrationExchangeItemKey = "__Dbsc.RegistrationExchange";
    ...
    /// delegate-based sign-out wiring installed by <see cref="PostConfigureDbscCookieOptions"/>.
    /// </summary>
