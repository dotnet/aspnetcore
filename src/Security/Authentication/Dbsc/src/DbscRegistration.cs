...
/// <summary>
/// Provides methods for writing Device Bound Session Credentials (DBSC) registration responses.
/// </summary>
[Experimental("ASP0031", UrlFormat = "https://aka.ms/aspnet/analyzer/{0}")]
public static class DbscRegistration
{
    /// <summary>
    ...
    /// <param name="dbscScheme">The registered DBSC authentication scheme.</param>
    /// <exception cref="ArgumentNullException"><paramref name="context"/> is <see langword="null"/>.</exception>
    /// <exception cref="ArgumentException"><paramref name="dbscScheme"/> is <see langword="null"/> or empty.</exception>
    /// <exception cref="InvalidOperationException">
    /// <paramref name="dbscScheme"/> is not a registered DBSC scheme, or the current user is not authenticated.
    /// </exception>
    /// <example>
    ...
    /// <code>
    /// // The default DBSC bound session cookie follows .AspNetCore.{dbscScheme}.Session
    /// const string boundCookie = ".AspNetCore.DBSC.Session";
    ///
    ...
    public static async Task WriteRegistrationAsync(HttpContext context, string dbscScheme)
    {
        ArgumentNullException.ThrowIfNull(context);
        ArgumentException.ThrowIfNullOrEmpty(dbscScheme);

        var sourceSchemes = context.RequestServices
            .GetRequiredService<IOptions<DbscSourceSchemes>>().Value;
        if (!sourceSchemes.DbscSchemes.Contains(dbscScheme))
        {
            throw new InvalidOperationException(
                $"'{dbscScheme}' is not a registered DBSC scheme. " +
                $"Call AddDbsc(\"{dbscScheme}\") to register it first.");
        }
        ...
        {
            throw new InvalidOperationException(
                "The DBSC registration challenge must be bound to an authenticated principal.");
        }

        await DbscRegistrationHeader.Emit(context, principal, properties: null, dbscScheme);
    }
}
