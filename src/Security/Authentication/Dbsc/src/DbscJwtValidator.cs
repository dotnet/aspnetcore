...
/// </summary>
internal sealed class DbscJwtValidator
{
    private static readonly JsonWebTokenHandler _tokenHandler = new();
    private readonly ILogger<DbscJwtValidator> _logger;

    public DbscJwtValidator(ILogger<DbscJwtValidator> logger)
    {
        _logger = logger;
    }
    ...
    public async Task<DbscJwtResult?> ValidateAsync(string jwt, string? publicKeyJwk)
    {
        JsonWebToken token;
        try
        {
    ...
    private static SecurityKey? CreateSecurityKey(string jwkJson, string algorithm)
    {
        ...
        {
            DbscConstants.Es256 when string.Equals(jsonWebKey.Kty, "EC", StringComparison.Ordinal) && string.Equals(jsonWebKey.Crv, "P-256", StringComparison.Ordinal) => jsonWebKey,
            DbscConstants.Rs256 when string.Equals(jsonWebKey.Kty, "RSA", StringComparison.Ordinal) => jsonWebKey,
            _ => null,
        };
    }
}
