...
namespace TriageBuildFailures.GitHub
{
    ...
    {
        public static async Task CommentOnTest(this GitHubClientWrapper gitHubClient, TeamCityBuild build, GitHubIssue issue, List<string> testNames)
        {
            ...
            var branchLabel = GitHubUtils.GetBranchLabel(build.BranchName);
        ...
        public static async Task CommentOnBuild(this GitHubClientWrapper gitHubClient, TeamCityBuild build, GitHubIssue issue, string buildName)
        {
            var (commentsAboutThisBuild, commentsFromToday) = await gitHubClient.GatherComments(build, issue);
            if (!commentsAboutThisBuild.Any())
            {
        ...
        private static async Task<(IEnumerable<IssueComment> commentsAboutThisBuild, IEnumerable<IssueComment> commentsFromToday)>
            GatherComments(this GitHubClientWrapper gitHubClient, TeamCityBuild build, GitHubIssue issue)
        {
