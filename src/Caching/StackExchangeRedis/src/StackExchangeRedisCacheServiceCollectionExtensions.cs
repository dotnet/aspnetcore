...
public static class StackExchangeRedisCacheServiceCollectionExtensions
{
    ...
    /// </summary>
    /// <param name="services">The <see cref="IServiceCollection" /> to add services to.</param>
    /// <param name="setupAction">An <see cref="Action{RedisCacheOptions, TDependency}"/> to configure the provided
    /// <see cref="RedisCacheOptions"/> with a desired <typeparamref name="TDependency"/> resolved from the <see cref="IServiceProvider"/>.</param>
    /// <returns>The <see cref="IServiceCollection"/> so that additional calls can be chained.</returns>
