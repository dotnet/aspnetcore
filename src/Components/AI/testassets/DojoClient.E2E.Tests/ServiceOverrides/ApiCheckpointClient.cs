...
// Releases the checkpoint gates in whichever host owns RecordedChatClient.
//
...
internal sealed class ApiCheckpointClient
{
    ...
    public ApiCheckpointClient(ServerInstance server)
    {
    ...
    public async Task ReleaseAsync(string lastUserMessage, string frameName)
    {
        ...
        using var response = await client.PostAsync(
            $"{_server.AppUrl}/_test/lock/release?key={Uri.EscapeDataString(key)}",
            content: null);
