...
public class InputNumberTest
{
    ...
    public async Task DefaultStepAttributeIsAny()
    {
        ...
        var stepAttribute = frames.Array.Single(f => f.FrameType == RenderTreeFrameType.Attribute && f.AttributeName == "step");
    ...
    [Fact]
    public void ParsesShortMinValue()
    {
        var component = new StandaloneInputNumber<short>();
        component.SetFieldName("ShortMin");

        var success = component.TryParseValue(short.MinValue.ToString(CultureInfo.InvariantCulture), out var result, out _);
    ...
    [Fact]
    public void ParsesShortMaxValue()
    {
        var component = new StandaloneInputNumber<short>();
        component.SetFieldName("ShortMax");

        var success = component.TryParseValue(short.MaxValue.ToString(CultureInfo.InvariantCulture), out var result, out _);
    ...
    [Fact]
    public void FormatsShortMinValue()
    {
        var formatted = FormatValue((short)short.MinValue);
        Assert.Equal(short.MinValue.ToString(CultureInfo.InvariantCulture), formatted);
    }

    [Fact]
    public void FormatsShortMaxValue()
    {
        var formatted = FormatValue((short)short.MaxValue);
        Assert.Equal(short.MaxValue.ToString(CultureInfo.InvariantCulture), formatted);
    }
    ...
    public void ParsesFloatNegativeValue()
    {
        ...
        var success = component.TryParseValue("-34000000000", out var result, out _);

        Assert.True(success);
        Assert.Equal(-34000000000f, result);
    }
    ...
    [Fact]
    public void ParsesNullableShortWithValidInput()
    {
        var component = new StandaloneInputNumber<short?>();
        component.SetFieldName("NullableShort");
    ...
    public async Task ValueChangedCallbackNotFiredOnInvalidInput()
    {
        ...
        Assert.Equal(0, model.SomeNumber); // Model unchanged
    }
    ...
    public async Task CustomAttributePreservationInRenderTree()
    {
        ...
        var testIdAttribute = frames.Array.Single(f =>
            f.FrameType == RenderTreeFrameType.Attribute &&
            f.AttributeName == "data-testid");

        var autocompleteAttribute = frames.Array.Single(f =>
            f.FrameType == RenderTreeFrameType.Attribute &&
    ...
    public async Task ValueAttributeReflectsCurrentValue()
    {
        ...
        var valueAttribute = frames.Array.Single(f =>
            f.FrameType == RenderTreeFrameType.Attribute &&
    ...
    public async Task DefaultAttributesRenderedWithCustomAttributes()
    {
        ...
        var stepAttribute = frames.Array.Single(f =>
            f.FrameType == RenderTreeFrameType.Attribute &&
            f.AttributeName == "step");

        // Verify custom attributes present
        var minAttribute = frames.Array.Single(f =>
            f.FrameType == RenderTreeFrameType.Attribute &&
            f.AttributeName == "min");

        var maxAttribute = frames.Array.Single(f =>
            f.FrameType == RenderTreeFrameType.Attribute &&
            f.AttributeName == "max");
