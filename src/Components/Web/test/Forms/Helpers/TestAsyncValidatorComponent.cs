...
internal sealed class TestAsyncValidatorComponent : ComponentBase, IDisposable
{
    private TestAsyncValidator _impl;
    ...
    [Parameter]
    public EventCallback<TestAsyncValidator> Created { get; set; }

    protected override void OnInitialized()
    {
        if (EditContext is null)
        {
            throw new InvalidOperationException("Requires cascading EditContext.");
        }

        _impl = new TestAsyncValidator(EditContext);
        Configure?.Invoke(_impl);
        _ = Created.InvokeAsync(_impl);
    }
