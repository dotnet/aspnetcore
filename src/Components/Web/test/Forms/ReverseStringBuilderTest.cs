...
public class ReverseStringBuilderTest
{
    ...
    public void ToString_Works_WhenOnlyUsingStackAllocatedBuffer()
    {
        ...
        Assert.Equal(-1, builder.RentedMemoryIndex);
    }
    ...
    public void ToString_Works_AfterExceedingStackAllocatedBuffer()
    {
        ...
        // Assert
        Assert.Equal("Hello, world!", result);
        Assert.Equal(0, builder.RentedMemoryIndex);
    }

    [Fact]
    public void ToString_Works_AfterExpandingIntoMultipleBuffersFromStackAllocatedBuffer()
    {
        // Arrange
        Span<char> initialBuffer = stackalloc char[8];
        using var builder = new ReverseStringBuilder(initialBuffer);
        var padding = new string('A', ReverseStringBuilder.MinimumRentedMemorySize + 10);
    ...
    {
        // Arrange
        using var builder = new ReverseStringBuilder(ReverseStringBuilder.MinimumRentedMemorySize);
        var slotCount = ReverseStringBuilder.FixedMemorySlotLength + 3;
        ...
        for (var i = 0; i < slotCount; i++)
        {
            ...
            expected = new string(c, ReverseStringBuilder.MinimumRentedMemorySize) + expected;
            ...
            var s = new string(c, ReverseStringBuilder.MinimumRentedMemorySize - 1);
