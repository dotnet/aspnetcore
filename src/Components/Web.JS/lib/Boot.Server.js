...
import { discoverComponents, discoverPersistedState } from './Services/ComponentDescriptorDiscovery';
...
let started = false;
let connection;
async function boot(userOptions) {
    if (started) {
    ...
    Blazor.reconnect = async (existingConnection) => {
        if (renderingFailed) {
    ...
    const components = discoverComponents(document, 'server');
    const appState = discoverPersistedState(document);
    const circuit = new CircuitDescriptor(components, appState || '');
    // Configure navigation via SignalR
    Blazor._internal.navigationManager.listenForNavigationEvents((uri, state, intercepted) => {
    ...
    Blazor._internal.sendJSDataStream = (data, streamId, chunkSize) => sendJSDataStream(connection, data, streamId, chunkSize);
...
async function initializeConnection(options, logger, circuit) {
    var _a, _b;
    ...
    newConnection.on('JS.BeginTransmitStream', (streamId) => {
        const readableStream = new ReadableStream({
            start(controller) {
                newConnection.stream('SendDotNetStreamToJS', streamId).subscribe({
                    next: (chunk) => controller.enqueue(chunk),
    ...
    newConnection.on('JS.RenderBatch', (batchId, batchData) => {
    ...
    try {
        ...
        unhandledError(newConnection, ex, logger);
    ...
    if ((_b = (_a = newConnection.connection) === null || _a === void 0 ? void 0 : _a.features) === null || _b === void 0 ? void 0 : _b.inherentKeepAlive) {
    ...
    DotNet.attachDispatcher({
        ...
        endInvokeJSFromDotNet: (asyncHandle, succeeded, argsJson) => {
