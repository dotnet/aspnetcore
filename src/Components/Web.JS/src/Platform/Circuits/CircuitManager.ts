...
export class CircuitManager implements DotNet.DotNetCallDispatcher {
  ...
  // Count of in-flight circuit operations (async JS<->.NET interop calls and data
  // streams). Used to raise the generic 'circuitactivitychanged' event so external
  // features (e.g. the auto-pause package) can tell when the circuit is idle.
  ...
  private activityTrackingEnabled(): boolean {
    // Tracking is armed lazily: we only allocate per-operation closures when an
    // external listener (e.g. the auto-pause package) is observing circuit activity,
    // keeping the common no-listener path allocation-free.
