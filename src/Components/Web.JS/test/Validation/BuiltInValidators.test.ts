...
describe('numberValidator (MVC-specific)', () => {
  function ctx(value: string | null | undefined): ValidationContext {
    return makeContext({ value });
  }

  describe('empty values are not validated', () => {
    test('accepts empty string', () => {
      expect(numberValidator(ctx(''))).toBe(true);
    });

    test('accepts null', () => {
      expect(numberValidator(ctx(null))).toBe(true);
    });

    test('accepts undefined', () => {
      expect(numberValidator(ctx(undefined))).toBe(true);
    });
  });

  describe('valid numbers', () => {
    test('accepts integer', () => {
      expect(numberValidator(ctx('42'))).toBe(true);
    });

    test('accepts negative integer', () => {
      expect(numberValidator(ctx('-5'))).toBe(true);
    });

    test('accepts decimal', () => {
      expect(numberValidator(ctx('3.14'))).toBe(true);
    });

    test('accepts negative decimal', () => {
      expect(numberValidator(ctx('-0.5'))).toBe(true);
    });

    test('accepts zero', () => {
      expect(numberValidator(ctx('0'))).toBe(true);
    });
  });

  describe('invalid numbers', () => {
    test('rejects alphabetic string', () => {
      expect(numberValidator(ctx('abc'))).toBe(false);
    });

    test('rejects mixed alphanumeric', () => {
      expect(numberValidator(ctx('123abc'))).toBe(false);
    });

    test('rejects multiple dots', () => {
      expect(numberValidator(ctx('1.2.3'))).toBe(false);
    });
  });
});
