...
namespace Microsoft.AspNetCore.Components.WebAssembly.DebugProxy
{
    public class Startup
    {
        ...
        private DebugProxyOptions options;
        ...
        public void Configure(IApplicationBuilder app, DebugProxyOptions debugProxyOptions)
        {
            ...
            {
                endpoints.MapGet("/", DebuggingHome);
                endpoints.MapGet("/_framework/debug/ws-proxy", WsProxy);
            });
        }

        private async Task WsProxy(HttpContext context)
        {
        ...
        private async Task DebuggingHome(HttpContext context)
        {
            ...
            var debuggerTabsListUrl = $"{options.BrowserHost}/json";
...
<pre>{ex}</pre>
        ...
        private string GetDevToolsUrlWithProxy(HttpRequest request, BrowserTab tabToDebug)
        {
            ...
            var devToolsUrlAbsolute = new Uri(options.BrowserHost + tabToDebug.DevtoolsFrontendUrl);
        ...
        private string GetLaunchChromeInstructions(string targetApplicationUrl)
        {
            ...
            var debuggerPort = new Uri(options.BrowserHost).Port;
        ...
        private async Task<IEnumerable<BrowserTab>> GetOpenedBrowserTabs()
        {
            ...
            var jsonResponse = await httpClient.GetStringAsync($"{options.BrowserHost}/json");
