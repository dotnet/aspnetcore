...
public class ProtectedBrowserStorageSerializerOptionsTest
{
    ...
    {
        var first = new StubResolver();
        var second = new StubResolver();

        var options = CreateOptions(new StubContext(first), new StubContext(second));

        Assert.Same(first, options.TypeInfoResolverChain[0]);
        Assert.Same(second, options.TypeInfoResolverChain[1]);
    }
    ...
    {
        public override IReadOnlyList<Infrastructure.ComponentDescriptor> Components => [];
