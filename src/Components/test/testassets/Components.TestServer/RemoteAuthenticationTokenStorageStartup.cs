...
public class RemoteAuthenticationTokenStorageStartup
{
    /// <summary>
    /// Base path under which the <c>TokenStorage = LocalStorage</c> WASM app is served.
    /// </summary>
    public const string TokenStorageBasePath = "/subdir-tokenstorage";

    /// <summary>
    /// Base path under which the <c>TokenStorage = SessionStorage</c> (default) WASM app is served.
    /// </summary>
    ...
    {
        app.Map(TokenStorageBasePath, app => ConfigureTokenStorageApp(app, env, TokenStorageBasePath));
        app.Map(SessionStorageBasePath, app => ConfigureSessionStorageApp(app, env, SessionStorageBasePath));
    }

    private static void ConfigureTokenStorageApp(IApplicationBuilder app, IWebHostEnvironment env, string basePath)
    {
        app.UseRouting();
        ...
        app.UseEndpoints(endpoints =>
        {
            ...
            endpoints.MapRazorComponents<RemoteAuthenticationTokenStorageApp>()
                .AddAdditionalAssemblies(Assembly.Load("Components.WasmRemoteAuthenticationTokenStorage"))
                .AddInteractiveWebAssemblyRenderMode(options => options.PathPrefix = "/WasmRemoteAuthenticationTokenStorage");

            MapOidcEndpoints(endpoints, basePath);
        });
    }

    private static void ConfigureSessionStorageApp(IApplicationBuilder app, IWebHostEnvironment env, string basePath)
    {
        app.UseRouting();
        ...
        app.UseEndpoints(endpoints =>
        {
            ...
            endpoints.MapRazorComponents<RemoteAuthenticationSessionStorageApp>()
                .AddAdditionalAssemblies(Assembly.Load("Components.WasmRemoteAuthenticationSessionStorage"))
                .AddInteractiveWebAssemblyRenderMode(options => options.PathPrefix = "/WasmRemoteAuthenticationSessionStorage");
