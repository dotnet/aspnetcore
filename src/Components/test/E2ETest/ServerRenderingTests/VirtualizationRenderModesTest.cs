...
public class VirtualizationRenderModesTest : ServerTestBase<BasicTestAppServerSiteFixture<RazorComponentEndpointsStartup<App>>>
{
    ...
    public void InitialItemIndex_NearEndResize_DoesNotApplyStaleRenderedWindowMeasurement()
    {
        ...
        Navigate($"{ServerPathBase}/interactivity/virtualization?test-stale-measurement=true&initial-item-index=1990");

        Browser.Exists(By.Id("scroll-container"));
        Browser.Exists(By.Id("blazor-started"));
        var js = (IJavaScriptExecutor)Browser;

        var states = new List<ViewportState>
        {
            WaitForViewportToSettle(js, "initial 2500px"),
        };

        for (var cycle = 1; cycle <= 3; cycle++)
        {
            SetContainerHeight(js, 2000);
            states.Add(WaitForViewportToSettle(js, $"cycle {cycle} at 2000px"));
            SetContainerHeight(js, 2500);
            states.Add(WaitForViewportToSettle(js, $"cycle {cycle} at 2500px"));
        }

        foreach (var state in states)
        {
            Assert.True(state.FirstVisibleItem == 1950
                && state.TopCovered
                && (state.BottomCovered || state.AtScrollEnd)
                && Math.Abs(state.ScrollHeight - 100_000) <= 2,
                $"Unexpected viewport state after {state.Stage}: {state}");
        }
    ...
    private ViewportState WaitForViewportToSettle(IJavaScriptExecutor js, string stage)
    {
        ViewportState? previous = null;
        var stableReads = 0;
        var current = default(ViewportState);

        Browser.True(() =>
        {
            current = GetViewportState(js) with { Stage = stage };
            if (previous is { } previousValue
                && current.FirstVisibleItem == previousValue.FirstVisibleItem
                && current.FirstRenderedItem == previousValue.FirstRenderedItem
                && current.LastRenderedItem == previousValue.LastRenderedItem
                && current.RenderedItemCount == previousValue.RenderedItemCount
                && current.ScrollHeight == previousValue.ScrollHeight
                && current.ScrollTop == previousValue.ScrollTop)
            {
                stableReads++;
            }
            else
            ...
            previous = current;
            return stableReads >= 5;
        }, TimeSpan.FromSeconds(15), $"Viewport did not settle after {stage}. Last state: {current}");

        return current;
    }

    private static ViewportState GetViewportState(IJavaScriptExecutor js)
    {
        var result = (Dictionary<string, object>)js.ExecuteScript(
            """
            const container = document.getElementById('scroll-container');
            const containerRect = container.getBoundingClientRect();
            const viewportTop = containerRect.top + container.clientTop;
            const viewportBottom = viewportTop + container.clientHeight;
            const items = Array.from(container.querySelectorAll('.item[data-index]'));
            const visibleItems = items.filter(item => {
                const rect = item.getBoundingClientRect();
                return rect.bottom > viewportTop + 1 && rect.top < viewportBottom - 1;
            });
            const topCovered = items.some(item => {
                const rect = item.getBoundingClientRect();
                return rect.top <= viewportTop + 1 && rect.bottom > viewportTop + 1;
            });
            const bottomCovered = items.some(item => {
                const rect = item.getBoundingClientRect();
                return rect.top < viewportBottom - 1 && rect.bottom >= viewportBottom - 1;
            });
            const index = item => item ? Number(item.getAttribute('data-index')) : -1;
            return {
                firstVisibleItem: index(visibleItems[0]),
                firstRenderedItem: index(items[0]),
                lastRenderedItem: index(items[items.length - 1]),
                renderedItemCount: items.length,
                scrollHeight: container.scrollHeight,
                scrollTop: container.scrollTop,
                topCovered,
                bottomCovered,
                atScrollEnd: Math.abs(container.scrollTop + container.clientHeight - container.scrollHeight) <= 2,
            };
            """);

        return new ViewportState(
            Stage: string.Empty,
            FirstVisibleItem: Convert.ToInt32(result["firstVisibleItem"], CultureInfo.InvariantCulture),
            ...
            ScrollHeight: Convert.ToDouble(result["scrollHeight"], CultureInfo.InvariantCulture),
            ScrollTop: Convert.ToDouble(result["scrollTop"], CultureInfo.InvariantCulture),
            ...
            AtScrollEnd: Convert.ToBoolean(result["atScrollEnd"], CultureInfo.InvariantCulture));
    }

    private static void SetContainerHeight(IJavaScriptExecutor js, int height)
    {
    ...
    private readonly record struct ViewportState(
        ...
        int FirstRenderedItem,
        int LastRenderedItem,
