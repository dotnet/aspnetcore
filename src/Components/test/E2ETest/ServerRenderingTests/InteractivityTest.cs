...
public class InteractivityTest : ServerTestBase<BasicTestAppServerSiteFixture<RazorComponentEndpointsStartup<App>>>
{
    ...
    public void Textarea_DynamicallyUpdatesOnNavigation()
    {
        ...
        Navigate($"{ServerPathBase}/textarea-method");
        ...
        // Simulate user editing the textarea and firing input event (which is what Blazor's @bind listens to)
        ((IJavaScriptExecutor)Browser).ExecuteScript("var ta = document.getElementById('textArea'); ta.value = 'User edited'; ta.dispatchEvent(new Event('input', { bubbles: true }));");
        // Verify the textarea value was updated
        Browser.Equal("User edited", () => (string)((IJavaScriptExecutor)Browser).ExecuteScript("return document.getElementById('textArea').value"));
        ...
        // Use client-side/enhanced navigation to method2 and check that textarea value updates to new method data
    ...
    [Fact]
    // Regression test for https://github.com/dotnet/aspnetcore/issues/45951
    // Blazor TextArea Not Updating
    //
    // This tests the component defined in: src/Components/RazorComponents/Pages/Index.razor (or similar page with @page "/")
    ...
    //
    // Issue: When a textarea has its content populated from a method using `<textarea>@GetData(Method)</textarea>`
    // pattern and user edits the textarea, then navigates to a different method parameter, the textarea should
    // update to the new method's data. However, it doesn't update - the textContent remains stale.
    // The textbox input with `value="@GetData(Method)"` works correctly by comparison.
    public void TextAreaNotUpdating_TextAreaWithMethodContent_ShouldUpdateWhenParameterChanges()
    {
        // Arrange: Start on the `/textarea-method` page and click Method 1 to render controls
        Navigate($"{ServerPathBase}/textarea-method");

        // Ensure enhanced (client-side) navigation is enabled for this test
        // EnhancedNavigationTestUtil.SuppressEnhancedNavigation(this, suppressEnhancedNavigation: false);
        ...
        // Verify initial state - textarea and textbox both show Method 1 data
        Browser.Equal("Initial value for method 1", () => (string)((IJavaScriptExecutor)Browser).ExecuteScript("return document.getElementById('textArea').value"));
        Browser.Equal("Initial value for method 1", () => Browser.FindElement(By.Id("textBox")).GetAttribute("value"));

        // Capture the original elements so we can assert enhanced navigation preserved them
        var originalTextArea = Browser.FindElement(By.Id("textArea"));
        var originalTextBox = Browser.FindElement(By.Id("textBox"));

        // Act: User edits the textarea
        ((IJavaScriptExecutor)Browser).ExecuteScript(
            "var ta = document.getElementById('textArea'); " +
            "ta.value = 'User edited method 1'; " +
            "ta.dispatchEvent(new Event('change'));");

        // Verify user's change was recorded
        Browser.Equal("User edited method 1", () => (string)((IJavaScriptExecutor)Browser).ExecuteScript("return document.getElementById('textArea').value"));

        // Act: User navigates to method2 by clicking the link (same-page navigation)
        Browser.Click(By.Id("method2-link"));

        // Verify that navigation was enhanced (did not perform a full-page reload)
        Browser.True(() => !originalTextArea.IsStale());

        // Assert: Textbox should update to Method 2 data (this works correctly)
        Browser.Equal("Method 2 initial value.", () => originalTextBox.GetAttribute("value"));

        // Assert: Textarea should also update to Method 2 data (this is the bug - #45951)
        // Expected: "Method 2 initial value."
        // Actual (buggy): "User edited method 1" (or whatever the user edited to)
        Browser.Equal("Method 2 initial value.", () => (string)((IJavaScriptExecutor)Browser).ExecuteScript("return document.getElementById('textArea').value"));
    }
