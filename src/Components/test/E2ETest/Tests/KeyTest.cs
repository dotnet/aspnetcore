...
namespace Microsoft.AspNetCore.Components.E2ETest.Tests
{
    public class KeyTest : BasicTestAppTestBase
    {
        ...
        public void CanInsert()
        {
            PerformTest(
                ...
                after: new[]
                {
                    ...
                    new Node("orig1", "A"),
                    new Node("new2", "Inserted between"),
                    new Node("orig1", "B"),
                    new Node("new3", "Inserted after"),
                });
        }
        ...
        private void PerformTest(Node[] before, Node[] after)
        {
            ...
            updateButton.Click();
            ValidateRenderedOutput(appElem, rootBefore);

            SetTextAreaValueFast(textbox, jsonAfter);
            updateButton.Click();
            ValidateRenderedOutput(appElem, rootAfter);
        }

        private static void ValidateRenderedOutput(IWebElement appElem, Node expectedRootNode)
        {
        ...
        private static void AssertNodesEqual(Node expectedRootNode, Node actualRootNode)
        {
            ...
            Assert.Collection(
                ...
                expectedRootNode.Children.Select<Node, Action<Node>>(expectedChild =>
                    (actualChild => AssertNodesEqual(expectedChild, actualChild))).ToArray());
        }
