...
internal sealed partial class PersistentServicesRegistry
{
    ...
    internal void RegisterForPersistence(PersistentComponentState state)
    {
        ...
        if (RenderMode != null)
        {
            subscriptions.Add((
                state.RegisterOnPersisting(() =>
                {
                    state.PersistAsJson(_registryKey, _registrations, RegistryJsonContext.Default.IPersistentServiceRegistrationArray);
                    return Task.CompletedTask;
                }, RenderMode),
                default));
        }
    ...
    private void PersistInstanceState(object instance, Type type, PersistentComponentState state)
    {
        ...
        {
            if (!PersistentStateSerializationOptions.CanSerialize(propertyType))
            {
                // Throwing here would abort the entire pause and leave the application with no persisted
                // state at all, so the property is skipped and reported instead.
    ...
    {
        if (state.TryTakeFromJson(_registryKey, RegistryJsonContext.Default.PersistentServiceRegistrationArray, out var registry) && registry != null)
        {
    ...
    // The registry itself is a framework DTO, so its contract is generated rather than reflected over.
