...
- **Maximize forward progress while asynchronous work is pending.** Concurrency means continuing independent work while an asynchronous operation is incomplete, not running component work on multiple threads. Schedule all available synchronous work before awaiting, and introduce an asynchronous barrier only when the next operation depends on its result or ordering. For example, an incomplete `SetParametersAsync` call for one component must not prevent the renderer from processing other ready components or renders. Apply the same principle to root-component additions, updates, and removals and to enhanced navigation: retain incomplete rendering or DOM-update operations for later observation, but do not await them before starting independent work.
