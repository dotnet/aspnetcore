...
- **Maximize forward progress while asynchronous work is pending.** Concurrency means continuing independent work while an asynchronous operation is incomplete, not running component work on multiple threads. Schedule all available synchronous work before awaiting, and introduce an asynchronous barrier only when the next operation depends on its result or ordering.
...
- **Use the renderer dispatcher as the concurrency boundary.** Access renderer-, circuit-, and component-owned mutable state through the owning dispatcher, and dispatch the complete state-changing operation rather than only its resulting render request. This provides logical serialization, not exclusive execution of an entire asynchronous operation: state must remain valid whenever an incomplete `await` yields control, and assumptions may need to be rechecked when execution resumes.
- **Treat the synchronization context as the ownership boundary.** Code executing within a Blazor synchronization context has exclusive logical ownership of its context-bound state. Preserve that model by using tasks and asynchronous control flow to coordinate incomplete work, ordering, and completion. Do not add locks, semaphores, wait handles, or concurrent collections for state confined to one synchronization context. Use independent synchronization only for state accessed outside the synchronization context or shared across renderers, circuits, or other concurrent owners.
