...
- **Treat the synchronization context as the ownership boundary.** Access renderer-, circuit-, and component-owned mutable state through the owning dispatcher, and coordinate context-bound work using tasks and asynchronous control flow. Dispatch the complete state-changing operation rather than only its resulting render request. Do not add locks, semaphores, or concurrent collections to state confined to one synchronization context. This provides logical serialization, not exclusive execution of an entire asynchronous operation: state must remain valid whenever an incomplete `await` yields control, and assumptions may need to be rechecked when execution resumes. Use independent synchronization only for state accessed outside the synchronization context or shared across renderers, circuits, or other concurrent owners.
