...
- **Use the renderer dispatcher as the concurrency boundary.** Access renderer-, circuit-, and component-owned mutable state through the owning dispatcher, coordinate dispatcher-bound work using tasks and asynchronous control flow, and dispatch the complete state-changing operation rather than only its resulting render request. Do not add locks, semaphores, wait handles, or concurrent collections for state confined to one dispatcher. This provides logical serialization, not exclusive execution of an entire asynchronous operation: state must remain valid whenever an incomplete `await` yields control, and assumptions may need to be rechecked when execution resumes. Use independent synchronization only for state accessed outside the dispatcher or shared across renderers, circuits, or other concurrent owners.
