...
- Before treating a combination of behaviors as a product defect, determine when each feature was introduced and identify any relevant compatibility switches. Verify that the configured switch values support the newer feature; compatibility switches preserve older behavior but do not guarantee that newer features work with that behavior. Report unsupported combinations explicitly instead of proposing product changes to make them work.
