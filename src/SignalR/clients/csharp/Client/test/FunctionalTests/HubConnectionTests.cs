...
namespace Microsoft.AspNetCore.SignalR.Client.FunctionalTests
{
    ...
    public class HubConnectionTests : FunctionalTestBase
    {
        ...
        [Fact]
        public async Task StreamBufferCapacityHubOptionIsEnforced()
        {
            var protocol = HubProtocols["json"];
            using (StartServer<Startup>(out var server))
            {
                var connection = CreateHubConnection(server.Url, "/stream", HttpTransportType.WebSockets, protocol, LoggerFactory);
                try
                {
                    await connection.StartAsync().OrTimeout();

                    var channel = Channel.CreateBounded<string>(5);
                    var secondChannel = Channel.CreateBounded<string>(5);
                    await connection.SendAsync("AcceptStream", channel.Reader, secondChannel.Reader).OrTimeout();

                    //var channel = await connection.StreamAsChannelAsync<string>("StreamEcho", channelWriter.Reader).OrTimeout();

                    await channel.Writer.WriteAsync("1").AsTask().OrTimeout();
                    //Assert.Equal("1", await channel.ReadAsync().AsTask().OrTimeout());
                    await channel.Writer.WriteAsync("2").AsTask().OrTimeout();
