...
internal sealed partial class OpenApiJsonSchema(OpenApiSchema? schema)
{
    ...
    internal sealed class JsonConverter : JsonConverter<OpenApiJsonSchema>
    {
        public override OpenApiJsonSchema? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            reader.Read();
            // SweepSchemaKeyword is a special property that indicates that the schema should removed
            // from the document. This is used to remove sub-schemas generated by System.Text.Json that
            // don't match OpenAPI's schema constraints.
            var schemaMarkedForSweep = false;
            do
            {
                switch (reader.TokenType)
                {
                    case JsonTokenType.PropertyName:
                        ...
                        if (propertyName == Microsoft.AspNetCore.OpenApi.OpenApiConstants.SweepSchemaKeyword)
                        {
                            schemaMarkedForSweep = true;
                            reader.Read();
                            continue;
                        }
                    ...
                    case JsonTokenType.EndObject:
                        return new OpenApiJsonSchema(schemaMarkedForSweep ? null : schema);
                    default:
                        continue;
                }
            } while (reader.Read());
