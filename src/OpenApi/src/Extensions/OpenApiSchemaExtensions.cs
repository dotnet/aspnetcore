...
internal static class OpenApiSchemaExtensions
{
    ...
    public static OpenApiSchema Clone(this OpenApiSchema schema)
    {
        return new OpenApiSchema
        {
            ...
            AllOf = schema.AllOf is not null ? new List<OpenApiSchema>(schema.AllOf.Select(s => s.Clone())) : null,
            OneOf = schema.OneOf is not null ? new List<OpenApiSchema>(schema.OneOf.Select(s => s.Clone())) : null,
            AnyOf = schema.AnyOf is not null ? new List<OpenApiSchema>(schema.AnyOf.Select(s => s.Clone())) : null,
