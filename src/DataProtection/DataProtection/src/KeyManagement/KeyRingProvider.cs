...
{
    internal sealed partial class KeyRingProvider : ICacheableKeyRingProvider, IKeyRingProvider
    {
        ...
        private CacheableKeyRing CreateCacheableKeyRingCore(DateTimeOffset now, IKey? keyJustAdded)
        {
            ...
            Log.PolicyResolutionStatesThatANewKeyShouldBeAddedToTheKeyRing(_logger);
            ...
            if (!_keyManagementOptions.AutoGenerateKeys)
            {
                ...
                if (keyToUse == null)
                {
                    Log.KeyRingDoesNotContainValidDefaultKey(_logger);
                    throw new InvalidOperationException(Resources.KeyRingProvider_NoDefaultKey_AutoGenerateDisabled);
                }
                else
                {
                    Log.UsingFallbackKeyWithExpirationAsDefaultKey(_logger, keyToUse.KeyId, keyToUse.ExpirationDate);
                    return CreateCacheableKeyRingCoreStep2(now, cacheExpirationToken, keyToUse, allKeys);
                }
            }
        ...
        private CacheableKeyRing CreateCacheableKeyRingCoreStep2(DateTimeOffset now, CancellationToken cacheExpirationToken, IKey defaultKey, IEnumerable<IKey> allKeys)
        {
            ...
            Log.UsingKeyAsDefaultKey(_logger, defaultKey.KeyId);
            ...
            try
            {
                ...
                if (acquiredLock)
                {
                    if (!forceRefresh)
                    {
                        ...
                        {
                            Log.ExistingCachedKeyRingIsExpired(_logger);
                        }
                    }
                    ...
                    catch (Exception ex)
                    {
                        if (existingCacheableKeyRing != null)
                        {
                            Log.ErrorOccurredWhileRefreshingKeyRing(_logger, ex);
                        }
                        else
                        {
                            Log.ErrorOccurredWhileReadingKeyRing(_logger, ex);
                        }
        ...
        private partial class Log
        {
            ...
            public static partial void UsingFallbackKeyWithExpirationAsDefaultKey(ILogger logger, Guid keyId, DateTimeOffset expirationDate);

            [LoggerMessage(2, LogLevel.Debug, "Using key {KeyId:B} as the default key.", EventName = "UsingKeyAsDefaultKey")]
            public static partial void UsingKeyAsDefaultKey(ILogger logger, Guid keyId);
            ...
            public static partial void ExistingCachedKeyRingIsExpired(ILogger logger);
            ...
            public static partial void ErrorOccurredWhileRefreshingKeyRing(ILogger logger, Exception exception);

            [LoggerMessage(48, LogLevel.Error, "An error occurred while reading the key ring.", EventName = "ErrorOccurredWhileReadingKeyRing")]
            public static partial void ErrorOccurredWhileReadingKeyRing(ILogger logger, Exception exception);
            ...
            public static partial void KeyRingDoesNotContainValidDefaultKey(ILogger logger);
            ...
            public static partial void PolicyResolutionStatesThatANewKeyShouldBeAddedToTheKeyRing(ILogger logger);
        }
    }
}
