...
namespace System.Buffers
{
    ...
    {
        private static readonly int s_shardCount = RoundUpToPowerOf2(Environment.ProcessorCount);
        private static readonly int s_shardMask = s_shardCount - 1;

        private readonly MemoryPoolShard[] s_shards = CreateShards();

        private int s_shardIndex = 0;

        private static MemoryPoolShard[] CreateShards()
        {
            var shards = new MemoryPoolShard[s_shardCount];
            for (int i = 0; i < shards.Length; i++)
            {
                shards[i] = new MemoryPoolShard();
            }
        ...
        public override IMemoryOwner<byte> Rent(int size = AnySize)
        {
            ...
            var roundRobinIndex = Interlocked.Increment(ref s_shardIndex) & s_shardMask;
            return s_shards[roundRobinIndex].Lease();
        }
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            lock (_disposeSync)
            {
                ...
                for (int i = 0; i < shards.Length; i++)
                {
                    shards[i].Dispose(disposing);
                }
            }
        }

        internal sealed class MemoryPoolShard
        {
            /// <summary>
