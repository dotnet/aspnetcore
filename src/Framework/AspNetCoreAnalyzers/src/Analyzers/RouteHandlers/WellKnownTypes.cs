...
internal sealed class WellKnownTypes
{
    public static bool TryCreate(Compilation compilation, [NotNullWhen(true)] out WellKnownTypes? wellKnownTypes)
    {
        ...
        if (compilation.GetBestTypeByMetadataName(EndpointRouteBuilderExtensions) is not { } endpointRouteBuilderExtensions)
        {
            return false;
        }

        const string Delegate = "System.Delegate";
        if (compilation.GetBestTypeByMetadataName(Delegate) is not { } @delegate)
        {
            return false;
        }

        const string IBinderTypeProviderMetadata = "Microsoft.AspNetCore.Mvc.ModelBinding.IBinderTypeProviderMetadata";
        if (compilation.GetBestTypeByMetadataName(IBinderTypeProviderMetadata) is not { } ibinderTypeProviderMetadata)
        {
            return false;
        }
        ...
        if (compilation.GetBestTypeByMetadataName(BindAttribute) is not { } bindAttribute)
        {
            return false;
        }

        const string IResult = "Microsoft.AspNetCore.Http.IResult";
        if (compilation.GetBestTypeByMetadataName(IResult) is not { } iResult)
        {
            return false;
        }

        const string IActionResult = "Microsoft.AspNetCore.Mvc.IActionResult";
        if (compilation.GetBestTypeByMetadataName(IActionResult) is not { } iActionResult)
        {
            return false;
        }

        const string IConvertToActionResult = "Microsoft.AspNetCore.Mvc.Infrastructure.IConvertToActionResult";
        if (compilation.GetBestTypeByMetadataName(IConvertToActionResult) is not { } iConvertToActionResult)
        {
            return false;
        }
