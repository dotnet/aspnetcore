...
public sealed class DirectTlsEndpointOptions
{
    ...
    /// <summary>
    /// The HTTP protocols offered on this endpoint. This drives the ALPN protocol list advertised during the
    /// handshake (for example, <c>h2</c> is offered when <see cref="HttpProtocols.Http2"/> is included).
    /// </summary>
    /// <remarks>
    /// Defaults to <see cref="HttpProtocols.Http1AndHttp2"/>. HTTP/3 is not supported by this transport.
    /// </remarks>
    public HttpProtocols HttpProtocols { get; set; } = HttpProtocols.Http1AndHttp2;
