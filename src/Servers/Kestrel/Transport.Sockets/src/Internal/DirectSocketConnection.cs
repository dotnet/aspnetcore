...
/// <summary>
/// A socket connection that integrates OpenSSL directly with the socket,
/// eliminating data copies from socket->buffer->MemBIO->OpenSSL pipeline.
/// This transport handles TLS encryption/decryption at the transport layer.
/// </summary>
internal sealed partial class DirectSocketConnection : TransportConnection
{
    ...
    internal DirectSocketConnection(Socket socket,
    ...
    /// <summary>
    /// Gets a value indicating whether this connection uses direct OpenSSL integration.
    /// </summary>
    public bool IsDirectSSLConnection => _sslContext is not null;
    ...
    public void Start()
    {
        ...
        {
            _logger.LogError(0, ex, $"Unexpected exception in {nameof(DirectSocketConnection)}.{nameof(Start)}.");
        }
    }
    ...
    public override async ValueTask DisposeAsync()
    {
        ...
        {
            _logger.LogError(0, ex, $"Unexpected exception in {nameof(DirectSocketConnection)}.{nameof(DisposeAsync)}.");
        }
        finally
        {
            _receiver.Dispose();
    ...
    private async Task DoReceive()
    {
        ...
        try
        {
            if (IsDirectSSLConnection)
            {
                await DoReceiveDirect();
            }
            else
            {
                await DoReceiveStandard();
            }
        }
        catch (ObjectDisposedException ex)
        {
            ...
            if (_shutdownReason is not null)
            {
                ...
                _logger.LogError(0, error, "ObjectDisposedException in receive loop");
            }
        }
        catch (Exception ex)
        {
            // This is unexpected.
            error = ex;
            _logger.LogError(0, error, "Unexpected exception in receive loop");
        }
        finally
        {
    ...
    private async Task DoReceiveStandard()
    {
        while (_shutdownReason is null)
        {
            ...
            {
                // FIN
                _logger.LogDebug("Connection read FIN");
                break;
            }
            ...
            if (paused)
            {
                _logger.LogDebug("Connection paused");
            }
            ...
            if (paused)
            {
                _logger.LogDebug("Connection resumed");
            }
        ...
        bool IsNormalCompletion(SocketOperationResult result)
        {
            ...
            if (IsConnectionResetError(result.SocketError.SocketErrorCode))
            {
                ...
                _logger.LogDebug(ex, "Connection reset");
            ...
            {
                _logger.LogDebug(result.SocketError, "Connection abort error");

                return false;
            }

            // This is unexpected.
            _logger.LogError(result.SocketError, "Unexpected socket error");

            return false;
        }
    }

    private async Task DoReceiveDirect()
    {
        // Direct OpenSSL receive handling - data flows directly from socket to OpenSSL
        // without intermediate buffering
        if (_sslContext is null)
        {
            throw new InvalidOperationException("SSL context is required for direct receive");
        }
        ...
        while (_shutdownReason is null)
        {
            try
            {
                ...
                {
                    var waitForDataResult = await _receiver.WaitForDataAsync(_socket);
                    if (!IsDirectNormalCompletion(waitForDataResult))
                    {
                        break;
                    }
                }

                // For direct SSL, we still need to read from socket but OpenSSL handles decryption
                var buffer = Input.GetMemory(MinAllocBufferSize);
                var receiveResult = await _receiver.ReceiveAsync(_socket, buffer);

                if (!IsDirectNormalCompletion(receiveResult))
                {
                    break;
                }
                ...
                if (bytesReceived == 0)
                {
                    _logger.LogDebug("Direct SSL connection read FIN");
                    break;
                }
                ...
                if (paused)
                {
                    _logger.LogDebug("Direct SSL connection paused");
                }
                ...
                if (paused)
                {
                    _logger.LogDebug("Direct SSL connection resumed");
                }
            ...
            {
                _logger.LogError(ex, "Error in direct SSL receive");
                throw;
            }
        }

        bool IsDirectNormalCompletion(SocketOperationResult result)
        {
            if (_shutdownReason is not null)
            {
                return false;
            ...
            {
                var ex = result.SocketError;
                _logger.LogDebug(ex, "Direct SSL connection reset");
                return false;
            }

            if (IsConnectionAbortError(result.SocketError.SocketErrorCode))
            {
                _logger.LogDebug(result.SocketError, "Direct SSL connection abort error");
                return false;
            }

            _logger.LogError(result.SocketError, "Direct SSL unexpected socket error");
            return false;
        }
    }
    ...
    private async Task DoSend()
    {
                ...
                if (!buffer.IsEmpty)
                {
                    ...
                    if (transferResult.HasError)
                    {
                        if (IsConnectionResetError(transferResult.SocketError.SocketErrorCode))
                        {
                            ...
                            _logger.LogDebug(ex, "Connection reset on send");
        ...
        catch (Exception ex)
        {
            shutdownReason = ex;
            unexpectedError = ex;
            _logger.LogError(0, ex, "Unexpected exception in send loop");
        }
        finally
        {
    ...
    private void Shutdown(Exception? shutdownReason)
    {
        lock (_shutdownLock)
        {
            ...
            {
                _logger.LogDebug(shutdownReason, "Connection write RST");
            ...
            _logger.LogDebug(_shutdownReason.Message);
    ...
    private void CancelConnectionClosedToken()
    {
        ...
        {
            _logger.LogError(0, ex, $"Unexpected exception in {nameof(DirectSocketConnection)}.{nameof(CancelConnectionClosedToken)}.");
        }
    }
