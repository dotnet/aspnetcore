...
internal abstract class Http3ControlStream : IHttp3Stream, IThreadPoolWorkItem
{
            ...
            switch (_headerType)
            {
                case ControlStreamTypeId:
                    if (!_context.StreamLifetimeHandler.OnInboundControlStream(this))
                    {
                        ...
                        throw new Http3ConnectionErrorException(CoreStrings.FormatHttp3ControlStreamErrorMultipleInboundStreams("control"), Http3ErrorCode.StreamCreationError, ConnectionErrorReason.StreamCreationError);
                    }
                ...
                case EncoderStreamTypeId:
                    if (!_context.StreamLifetimeHandler.OnInboundEncoderStream(this))
                    {
                        ...
                        throw new Http3ConnectionErrorException(CoreStrings.FormatHttp3ControlStreamErrorMultipleInboundStreams("encoder"), Http3ErrorCode.StreamCreationError, ConnectionErrorReason.StreamCreationError);
                    }
                ...
                case DecoderStreamTypeId:
                    ...
                    {
                        // https://quicwg.org/base-drafts/draft-ietf-quic-qpack.html#section-4.2
                        throw new Http3ConnectionErrorException(CoreStrings.FormatHttp3ControlStreamErrorMultipleInboundStreams("decoder"), Http3ErrorCode.StreamCreationError, ConnectionErrorReason.StreamCreationError);
                    }
                    await HandleEncodingDecodingTask();
                    break;
                default:
    ...
    private ValueTask ProcessHttp3ControlStream(in ReadOnlySequence<byte> payload)
    {
        switch (_incomingFrame.Type)
        {
            ...
            case Http3FrameType.PushPromise:
                ...
                throw new Http3ConnectionErrorException(CoreStrings.FormatHttp3ErrorUnsupportedFrameOnControlStream(_incomingFrame.FormattedType), Http3ErrorCode.UnexpectedFrame, ConnectionErrorReason.UnexpectedFrame);
    ...
    private ValueTask ProcessSettingsFrameAsync(ReadOnlySequence<byte> payload)
    {
        if (_haveReceivedSettingsFrame)
        {
            ...
            throw new Http3ConnectionErrorException(CoreStrings.Http3ErrorControlStreamMultipleSettingsFrames, Http3ErrorCode.UnexpectedFrame, ConnectionErrorReason.UnexpectedFrame);
        }
    ...
    private void ProcessSetting(long id, long value)
    {
        ...
        switch (id)
        {
            ...
            case 0x5:
                ...
                throw new Http3ConnectionErrorException(message, Http3ErrorCode.SettingsError, ConnectionErrorReason.InvalidSettings);
    ...
    private void EnsureSettingsFrame(Http3FrameType frameType)
    {
        if (!_haveReceivedSettingsFrame)
        {
            ...
            throw new Http3ConnectionErrorException(message, Http3ErrorCode.MissingSettings, ConnectionErrorReason.InvalidSettings);
        }
    }
