...
{
    internal class MultiplexedConnectionManager
    {
        private readonly ConcurrentDictionary<long, MultiplexedConnectionReference> _connectionReferences = new ConcurrentDictionary<long, MultiplexedConnectionReference>();
        private readonly IKestrelTrace _trace;

        public MultiplexedConnectionManager(IKestrelTrace trace)
        {
            _trace = trace;
        }

        public void AddConnection(long id, MultiplexedKestrelConnection connection)
        {
            if (!_connectionReferences.TryAdd(id, new MultiplexedConnectionReference(connection)))
            {
        ...
        public void Walk(Action<MultiplexedKestrelConnection> callback)
        {
