...
{
    public class MultiplexedConnectionContextTests
    {
        [Fact]
        public void ParameterlessAbortCreateConnectionAbortedException()
        {
            var mockConnectionContext = new Mock<MultiplexedConnectionContext> { CallBase = true };
            ...
            Assert.NotNull(ex);
            Assert.Equal("The connection was aborted by the application via MultiplexedConnectionContext.Abort().", ex.Message);
        }
    }
}
