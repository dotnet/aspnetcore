...
namespace Microsoft.AspNetCore.Server.Kestrel.Transport.Quic.Internal
{
    internal class QuicTrace : IQuicTrace
    {
        private static readonly Action<ILogger, string, Exception?> _acceptedConnection =
            LoggerMessage.Define<string>(LogLevel.Debug, new EventId(1, "AcceptedConnection"), @"Connection id ""{ConnectionId}"" accepted.", new LogDefineOptions() { SkipEnabledCheck = true });
        private static readonly Action<ILogger, string, StreamType, Exception?> _acceptedStream =
            LoggerMessage.Define<string, StreamType>(LogLevel.Debug, new EventId(2, "AcceptedStream"), @"Stream id ""{ConnectionId}"" type {StreamType} accepted.", new LogDefineOptions() { SkipEnabledCheck = true });
        private static readonly Action<ILogger, string, StreamType, Exception?> _connectedStream =
            LoggerMessage.Define<string, StreamType>(LogLevel.Debug, new EventId(3, "ConnectedStream"), @"Stream id ""{ConnectionId}"" type {StreamType} connected.", new LogDefineOptions() { SkipEnabledCheck = true });
        private static readonly Action<ILogger, string, Exception?> _connectionError =
            LoggerMessage.Define<string>(LogLevel.Debug, new EventId(4, "ConnectionError"), @"Connection id ""{ConnectionId}"" unexpected error.", new LogDefineOptions() { SkipEnabledCheck = true });
        private static readonly Action<ILogger, string, Exception?> _connectionAborted =
            LoggerMessage.Define<string>(LogLevel.Debug, new EventId(5, "ConnectionAborted"), @"Connection id ""{ConnectionId}"" aborted by peer.", new LogDefineOptions() { SkipEnabledCheck = true });
        private static readonly Action<ILogger, string, string, Exception?> _connectionAbort =
            LoggerMessage.Define<string, string>(LogLevel.Debug, new EventId(6, "ConnectionAbort"), @"Connection id ""{ConnectionId}"" aborted by application because: ""{Reason}"".", new LogDefineOptions() { SkipEnabledCheck = true });
        private static readonly Action<ILogger, string, Exception?> _streamError =
            LoggerMessage.Define<string>(LogLevel.Debug, new EventId(7, "StreamError"), @"Stream id ""{ConnectionId}"" unexpected error.", new LogDefineOptions() { SkipEnabledCheck = true });
        private static readonly Action<ILogger, string, Exception?> _streamPause =
            LoggerMessage.Define<string>(LogLevel.Debug, new EventId(8, "StreamPause"), @"Stream id ""{ConnectionId}"" paused.", new LogDefineOptions() { SkipEnabledCheck = true });
        private static readonly Action<ILogger, string, Exception?> _streamResume =
            LoggerMessage.Define<string>(LogLevel.Debug, new EventId(9, "StreamResume"), @"Stream id ""{ConnectionId}"" resumed.", new LogDefineOptions() { SkipEnabledCheck = true });
        private static readonly Action<ILogger, string, string, Exception?> _streamShutdownWrite =
            LoggerMessage.Define<string, string>(LogLevel.Debug, new EventId(10, "StreamShutdownWrite"), @"Stream id ""{ConnectionId}"" shutting down writes because: ""{Reason}"".", new LogDefineOptions() { SkipEnabledCheck = true });
        private static readonly Action<ILogger, string, Exception?> _streamAborted =
            LoggerMessage.Define<string>(LogLevel.Debug, new EventId(11, "StreamAborted"), @"Stream id ""{ConnectionId}"" aborted by peer.", new LogDefineOptions() { SkipEnabledCheck = true });
        private static readonly Action<ILogger, string, string, Exception?> _streamAbort =
            LoggerMessage.Define<string, string>(LogLevel.Debug, new EventId(12, "StreamAbort"), @"Stream id ""{ConnectionId}"" aborted by application because: ""{Reason}"".", new LogDefineOptions() { SkipEnabledCheck = true });
