...
public class Http2TestBase : TestApplicationErrorLoggerLoggedTest, IDisposable, IHttpStreamHeadersHandler
{
    ...
    protected ConfiguredTaskAwaitable InitializeConnectionAsync(RequestDelegate application, int expectedSettingsCount = 3, bool expectedWindowUpdate = true, bool addKestrelFeatures = false) =>
        InnerInitializeConnectionAsync(application, expectedSettingsCount, expectedWindowUpdate).ConfigureAwait(false);

    private async Task InnerInitializeConnectionAsync(RequestDelegate application, int expectedSettingsCount, bool expectedWindowUpdate, bool addKestrelFeatures)
    {
