...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public async Task ThrowsAndClosesConnectionWhenAppWritesMoreThanContentLengthWrite()
        {
            ...
            var logMessage = Assert.Single(Messages, message => message.LogLevel == LogLevel.Error);
        ...
        public async Task WhenAppSetsContentLengthButDoesNotWriteBody500ResponseSentAndConnectionDoesNotClose()
        {
            ...
            var error = Messages.Where(message => message.LogLevel == LogLevel.Error);
        ...
        public async Task WhenAppSetsContentLengthToZeroAndDoesNotWriteNoErrorIsThrown(bool flushResponse)
        {
            ...
            Assert.Empty(Messages.Where(message => message.LogLevel == LogLevel.Error));
        }
...
#pragma warning disable CS0618 // Type or member is obsolete
            Assert.Contains(Messages, w => w.EventId.Id == 17 && w.LogLevel <= LogLevel.Debug && w.Exception is BadHttpRequestException
                && ((BadHttpRequestException)w.Exception).StatusCode == StatusCodes.Status400BadRequest);
#pragma warning restore CS0618 // Type or member is obsolete
        }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public async Task ThrowingResultsIn500Response()
        {
            ...
            Assert.False(onStartingCalled);
            Assert.Equal(2, Messages.Where(message => message.LogLevel == LogLevel.Error).Count());
        }
        ...
        public async Task ThrowingAfterWritingKillsConnection()
        {
            ...
            Assert.Single(Messages, message => message.LogLevel == LogLevel.Error);
        }
        ...
        public async Task AppAbortIsLogged()
        {
            ...
            Assert.Single(Messages.Where(m => m.Message.Contains(CoreStrings.ConnectionAbortedByApplication)));
        }
        ...
        public async Task AppAbortViaIConnectionLifetimeFeatureIsLogged()
        {
            ...
            Assert.Single(Messages.Where(m => m.Message.Contains("The connection was aborted by the application via IConnectionLifetimeFeature.Abort().")));
        }
