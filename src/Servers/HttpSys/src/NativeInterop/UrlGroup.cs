    ...
    internal class UrlGroup : IDisposable
    {
        ...
        internal void SetProperty(HttpApiTypes.HTTP_SERVER_PROPERTY property, IntPtr info, uint infosize, bool throwOnError = true)
        {
            ...
            if (statusCode != UnsafeNclNativeMethods.ErrorCodes.ERROR_SUCCESS)
            {
                ...
                _logger.LogError(LoggerEventIds.UrlGroupFailed, exception, "SetUrlGroupProperty");
                if (throwOnError)
                {
                    throw exception;
                }
        ...
        {
            _logger.LogDebug(LoggerEventIds.PrefixListened, "Listening on prefix: {0} " , uriPrefix);
            CheckDisposed();
        ...
        {
            _logger.LogInformation(LoggerEventIds.PrefixListenStopped, "Stop listening on prefix: {0} " , uriPrefix);
            CheckDisposed();
        ...
        public void Dispose()
        {
            ...
            if (statusCode != UnsafeNclNativeMethods.ErrorCodes.ERROR_SUCCESS)
            {
                _logger.LogError(LoggerEventIds.V2ConfigCleanupFailed, "CleanupV2Config; Result: {0}" , statusCode);
            }
            Id = 0;
        }
