...
// Example middleware exercising the generic async IHttpSysRequestPropertyFeature.TryGetRequestPropertyAsync API.
app.Use(async (context, next) =>
{
    const int HttpRequestPropertyTlsClientHello = 11;
    ...
    {
        // probe required size with empty output buffer and empty qualifier
        var probeTask = httpSysPropFeature.TryGetRequestPropertyAsync(
            HttpRequestPropertyTlsClientHello,
            qualifier: default,
            output: default);
        var probeWasSync = probeTask.IsCompletedSuccessfully;
        var probeResult = await probeTask;
        Debug.Assert(!probeResult.Succeeded);
        Debug.Assert(probeResult.BytesReturned > 0);

        var rented = ArrayPool<byte>.Shared.Rent(probeResult.BytesReturned);
        try
        {
            Memory<byte> output = rented.AsMemory(0, probeResult.BytesReturned);
            var fetchTask = httpSysPropFeature.TryGetRequestPropertyAsync(
                HttpRequestPropertyTlsClientHello,
                qualifier: default,
                output: output);
            var fetchWasSync = fetchTask.IsCompletedSuccessfully;
            var fetchResult = await fetchTask;
            Debug.Assert(fetchResult.Succeeded);

            await context.Response.WriteAsync(
                $"""
                    TryGetRequestPropertyAsync(HttpRequestPropertyTlsClientHello)
                    -------------------------------------------------------------
                    probe completed     = {(probeWasSync ? "synchronously" : "asynchronously (IOCP)")}
                    fetch completed     = {(fetchWasSync ? "synchronously" : "asynchronously (IOCP)")}
                    requiredSize        = {probeResult.BytesReturned}
                    bytesReturned       = {fetchResult.BytesReturned}
                    first 30 bytes      = {string.Join(' ', output.Span[..Math.Min(30, fetchResult.BytesReturned)].ToArray())}
    ...
    catch (Exception ex)
    {
        await context.Response.WriteAsync(
            $"""
                ...
                TryGetRequestPropertyAsync(HttpRequestPropertyTlsClientHello) threw: {ex.GetType().Name}: {ex.Message}
            """);
    }
