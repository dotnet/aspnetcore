...
IN_PROCESS_APPLICATION::LoadManagedApplication(ErrorContext& errorContext)
{
    ...
    if (waitResult == WAIT_TIMEOUT)
    {
        ...
        errorContext.generalErrorType = "ASP.NET Core Module Failed to Start Within Startup Time Limit";
        errorContext.errorReason = format("ASP.NET Core Module failed to start after %d milliseconds", m_pConfig->QueryStartupTimeLimitInMS());
