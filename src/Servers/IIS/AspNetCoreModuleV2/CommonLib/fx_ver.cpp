...
size_t index_of_non_numeric2(const std::wstring& str, size_t i)
{
    return str.find_first_not_of(TEXT("0123456789"), i);
}

bool try_stou2(const std::wstring& str, unsigned* num)
{
    if (str.empty())
    {
        return false;
    }
    if (index_of_non_numeric2(str, 0) != std::wstring::npos)
    {
        return false;
    }
...
fx_ver_asp_t::fx_ver_asp_t(int major, int minor, int patch, const std::wstring& pre, const std::wstring& build)
    : m_major(major)
    , m_minor(minor)
    , m_patch(patch)
    , m_pre(pre)
...
fx_ver_asp_t::fx_ver_asp_t(int major, int minor, int patch, const std::wstring& pre)
    : fx_ver_asp_t(major, minor, patch, pre, TEXT(""))
{
}

fx_ver_asp_t::fx_ver_asp_t(int major, int minor, int patch)
    : fx_ver_asp_t(major, minor, patch, TEXT(""), TEXT(""))
{
}

fx_ver_asp_t::fx_ver_asp_t()
    : fx_ver_asp_t(-1, -1, -1, TEXT(""), TEXT(""))
{
}

bool fx_ver_asp_t::operator ==(const fx_ver_asp_t& b) const
{
...
std::wstring fx_ver_asp_t::as_str() const
{
...
std::wstring fx_ver_asp_t::prerelease_glob() const
{
    std::wstringstream stream;
...
std::wstring fx_ver_asp_t::patch_glob() const
{
    std::wstringstream stream;
...
/* static */
int fx_ver_asp_t::compare(const fx_ver_asp_t& a, const fx_ver_asp_t& b)
{
    ...
    for (size_t i = idStart; true; ++i)
    {
        if (a.m_pre[i] != b.m_pre[i])
        {
            ...
            unsigned idanum = 0;
            bool idaIsNum = try_stou2(ida, &idanum);
            unsigned idbnum = 0;
            bool idbIsNum = try_stou2(idb, &idbnum);
...
static bool validIdentifier(const std::wstring& id, bool buildMeta)
{
    ...
    if (!buildMeta && id[0] == TEXT('0') && id[1] != 0 && index_of_non_numeric2(id, 1) == std::wstring::npos)
    {
...
bool parse_internal(const std::wstring& ver, fx_ver_asp_t* fx_ver, bool parse_only_production)
{
    size_t maj_start = 0;
    ...
    unsigned major = 0;
    if (!try_stou2(ver.substr(maj_start, maj_sep), &major))
    {
        return false;
    }
    ...
    unsigned minor = 0;
    if (!try_stou2(ver.substr(min_start, min_sep - min_start), &minor))
    {
        return false;
    }
    ...
    size_t pat_sep = index_of_non_numeric2(ver, pat_start);
    if (pat_sep == std::wstring::npos)
    {
        if (!try_stou2(ver.substr(pat_start), &patch))
        {
            return false;
        }
        ...
        *fx_ver = fx_ver_asp_t(major, minor, patch);
        return true;
    }
    ...
    if (!try_stou2(ver.substr(pat_start, pat_sep - pat_start), &patch))
    {
        return false;
    }
    ...
    *fx_ver = fx_ver_asp_t(major, minor, patch, pre, build);
    return true;
}

/* static */
bool fx_ver_asp_t::parse(const std::wstring& ver, fx_ver_asp_t* fx_ver, bool parse_only_production)
{
