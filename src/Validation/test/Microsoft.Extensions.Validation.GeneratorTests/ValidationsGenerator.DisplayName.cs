...
public class ValidationsGeneratorDisplayNameTests : ValidationsGeneratorTestBase
{
    ...
    public async Task PropertyDisplayName_WithResourceType()
    {
        ...
        {
            [Range(10, 100, ErrorMessage = "Name:{0}")]
            [Display(Name = "ValueDisplayName", ResourceType = typeof(TestResources))]
            public int Value { get; set; } = 10;
        }
    ...
    public async Task ParameterDisplayName_WithResourceType()
    {
        ...
        app.MapPost("/param-resource-display-name", (
            [Range(10, 100, ErrorMessage = "Name:{0}")]
            [Display(Name = "ParamDisplayName", ResourceType = typeof(ParamResources))]
            int value) => "OK");
...
public class TypeWithDisplayName : IValidatableObject
{
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext context)
    {
        if (Value < 10)
        {
            yield return new ValidationResult($"TypeName:{context.DisplayName}");
        }
    }
}
""";
        ...
        await VerifyEndpoint(compilation, "/type-display-name", async (endpoint, serviceProvider) =>
        {
            ...
            var problemDetails = await AssertBadRequest(context);
            var kvp = Assert.Single(problemDetails.Errors);
            Assert.Equal("", kvp.Key);
            Assert.Equal("TypeName:My Model", kvp.Value.Single());
        });
    }
...
""";
        ...
        await VerifyEndpoint(compilation, "/type-resource-display-name", async (endpoint, serviceProvider) =>
        {
            ...
            var kvp = Assert.Single(problemDetails.Errors);
            Assert.Equal("", kvp.Key);
            Assert.Equal("TypeName:Localized Type Name", kvp.Value.Single());
        });
    }
    ...
    public async Task RecordPropertyDisplayName_WithNameOnly()
    {
        ...
        public record RecordWithDisplayName(
            ...
            [property: Display(Name = "Record Field Label")]
            int Value);
        """;
        await Verify(source, out var compilation);
    ...
    {
        // Verifies the constructor-parameter-first lookup in DisplayAttributeCache:
        // record primary constructor parameters carry the [Display] attribute, not the
        // generated property. The runtime cache must check the constructor parameters first.
        var source = Preamble + """
        ...
        public record RecordWithResourceDisplayName(
            [property: Range(10, 100, ErrorMessage = "Name:{0}")]
            [Display(Name = "RecordValueDisplayName", ResourceType = typeof(RecordResources))]
            int Value);
    ...
    public async Task PropertyDisplayName_DisplayAttributeTakesPrecedenceOverDisplayNameAttribute()
    {
        ...
        {
            [Range(10, 100, ErrorMessage = "Name:{0}")]
            [Display(Name = "Display Attr Name")]
            [DisplayName("DisplayName Attr Name")]
            public int Value { get; set; } = 10;
        }
        """;
        await Verify(source, out var compilation);
    ...
    public async Task PropertyDisplayName_DisplayAttributeResourceTypeTakesPrecedenceOverDisplayNameAttribute()
    {
        ...
        {
            [Range(10, 100, ErrorMessage = "Name:{0}")]
            [Display(Name = "ValueDisplayName", ResourceType = typeof(PrecedenceResources))]
...
public class TypeWithDisplayNameAttr : IValidatableObject
{
    public int Value { get; set; } = 10;
...
""";
        ...
        await VerifyEndpoint(compilation, "/type-displayname-attr", async (endpoint, serviceProvider) =>
        {
            ...
            var kvp = Assert.Single(problemDetails.Errors);
            Assert.Equal("", kvp.Key);
            Assert.Equal("TypeName:Friendly Type", kvp.Value.Single());
        });
    }

    [Fact]
    public async Task PropertyDisplayName_WithControlCharacters_EmitsValidLiteral()
    {
        // Regression for #66646 review feedback: FormatNullableStringLiteral must safely escape
        // any character that would otherwise produce uncompilable C# source. The previous
        // implementation only escaped backslash and quote; values containing newlines, tabs, or
        // other control characters broke the generated file. SymbolDisplay.FormatLiteral is the
        // canonical full escaping helper used across the repo's source generators.
        var source = Preamble + """
        app.MapPost("/property-display-name-control-chars", (PropertyControlCharsType model) => Results.Ok("Passed"!));

        app.Run();

        public class PropertyControlCharsType
        {
            [Range(10, 100, ErrorMessage = "Name:{0}")]
            [Display(Name = "Line1\nLine2\tTabbed\rCR\"Quoted\"\\Slash")]
            public int Value { get; set; } = 10;
        }
        """;
        await Verify(source, out var compilation);
        await VerifyEndpoint(compilation, "/property-display-name-control-chars", async (endpoint, serviceProvider) =>
        {
            var payload = """{ "Value": 5 }""";
            var context = CreateHttpContextWithPayload(payload, serviceProvider);
            await endpoint.RequestDelegate(context);
            ...
            var kvp = Assert.Single(problemDetails.Errors);
            Assert.Equal("Value", kvp.Key);
            Assert.Equal("Name:Line1\nLine2\tTabbed\rCR\"Quoted\"\\Slash", kvp.Value.Single());
        });
    }
}
